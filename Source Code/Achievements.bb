...
Const Achv001% = 37, Achv005% = 38, Achv006% = 39, Achv009% = 40, Achv020% = 41, Achv038% = 42, Achv178% = 43, Achv409% = 44
Const Achv1074% = 45, Achv1079% = 46, Achv173troll% = 47
...
Else
	loc2% = GetINISectionLocation("ProjectR\Data\achievementstrings.ini", "s"+Str(i))
	AchievementStrings(i) = GetINIString2("ProjectR\Data\achievementstrings.ini", loc2, "string1")
	AchievementDescs(i) = GetINIString2("ProjectR\Data\achievementstrings.ini", loc2, "AchvDesc")
		
	image$ = GetINIString2("ProjectR\Data\achievementstrings.ini", loc2, "image") 
		
	AchvIMG(i) = LoadImage_Strict("ProjectR\GFX\menu\achievements\"+image+".jpg")
	AchvIMG(i) = ResizeImage2(AchvIMG(i),ImageWidth(AchvIMG(i))*GraphicHeight/768.0,ImageHeight(AchvIMG(i))*GraphicHeight/768.0)
 EndIf
...
Function GiveAchievement(achvname%, showMessage%=True)
	If Achievements(achvname)<>True Then
		...
		Else
			loc2% = GetINISectionLocation("ProjectR\Data\achievementstrings.ini", "s"+achvname)
			AchievementName$ = GetINIString2("ProjectR\Data\achievementstrings.ini", loc2, "string1")
			;Msg = "Achievement Unlocked - "+AchievementName
			;MsgTimer=70*7
			CreateAchievementMsg(achvname,AchievementName)			
		EndIf
...
Function UpdateAchievementMsg()
	...
	For amsg = Each AchievementMsg
		If amsg\msgtime <> 0
			...
			Color 255,255,255
			SetFont Font1
			RowText("成就解锁 - "+amsg\txt,x+84*scale,y+10*scale,width-94*scale,y-20*scale)
