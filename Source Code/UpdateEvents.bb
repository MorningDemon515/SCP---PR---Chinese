	...
	For e.Events = Each Events
		Select e\EventName
			...
			Case "room2cafeteria"
				...
				If e\EventState = 0 Then
					If Curr066 = Null Then ;Ensures that another 066 doesn't spawn
						Curr066 = CreateNPC(NPCtype066, EntityX(e\room\obj), 0.5, EntityZ(e\room\obj))
					Else
			            PositionEntity Curr066\Collider, EntityX(e\room\obj), 0.5, EntityZ(e\room\obj)	
					...
					Else
						...
						Else
							...
							If e\EventState < 70 Then 
								...
								If KillTimer => 0 And SCP173Mode = False Then 
									For i = 0 To 2
...
;						EndIf
			...
			Case "room2test1074"
				...
				If PlayerRoom=e\room Then
                    ...
                    If e\Sound=0 Then e\Sound=LoadSound_Strict("ProjectR\SFX\music\Blue Feather.ogg")

					If Condition113 = 1 And (Not ((Wearing714=1) Or (WearingNightVision>0))) Then ;Condition: Gender - Female, Not wearing 714 or NVG
					    aa=GetSurfaceBrush(e\room\NonFreeAble[0])
					    BrushTexture aa, e\room\Textures[2], 0, 2
					    PaintSurface e\room\NonFreeAble[0],aa
					    FreeBrush aa
					ElseIf ((Wearing714=1) Or (WearingNightVision>0)) Then ;Condition: Gender - Both, Wearing 714 or NVG
					    aa=GetSurfaceBrush(e\room\NonFreeAble[0])
					    BrushTexture aa, e\room\Textures[0], 0, 2										
					    PaintSurface e\room\NonFreeAble[0],aa
					    FreeBrush aa
					Else ;Condition: Gender - Male, Not Wearing 714 or NVG			
					    aa=GetSurfaceBrush(e\room\NonFreeAble[0])
					...
					If e\EventState > 0.0 Then
						HeartBeatRate = 150
						HeartBeatVolume = 0.5
						If e\EventState = 1.0 And (Not Condition113 = 1) Then e\SoundCHN = PlaySound_Strict(e\Sound) : ChannelVolume e\SoundCHN,1.0
						...
						Select Floor(e\EventState)
							Case 250.0
							If Juan = True Then
							    Msg = "Hey there!" ;Ensure the player see this bullshit								
								MsgTimer = 70*5								
							ElseIf Condition113 = 1 Then
							    Msg = "''What the? What kind of work art is this?''"
								MsgTimer = 70*5							
							Else
								Msg = "''This work of art is the greatest I've seen in my entire life.''" ;It looks beautiful. Don't tarnish the art of beauty.
								MsgTimer = 70*5
							EndIf	
							Case 500.0
							If Juan = True Then
								Msg = "If you're seeing this, post a bug report." ;Regalis will be angry.								
								MsgTimer = 70*5								
							ElseIf Condition113 = 1 Then
							    Msg = "''I feel uncomfortable. This spiral-''"
								MsgTimer = 70*5							
							Else	
								Msg = "''But why do I see myself in it?''"	;Juan, that is you in the canvas. No wonder "you" in the canvas displayed anger.						
								MsgTimer = 70*5
							EndIf								
							Case 700.0
							If Juan = True Then 
							    Msg = "Seriously, this SCP should not spawn." ;I was fine with this SCP until SCP-1162 came in, I like SCP-1162 more than this shitty canvas.						
								MsgTimer = 70*5								
							ElseIf Condition113 = 1 Then
							    Msg = "''It is... luring me in. But why do I see myself in it?''"
								MsgTimer = 70*5							
							Else
								Msg = "''What is this supposed to mean?''" ;No SCPs should deserve the hate. Couldn't you just chill instead? 	
								MsgTimer = 70*5
							EndIf								
							Case 800.0
								BlinkTimer = -2
							...
							Case 1200.0
							If Juan = True Then 
							    Msg = "This event sucks." ;Let's agree this event was fucking horribly implemented.							
								MsgTimer = 70*5								
							ElseIf Condition113 = 1 Then
							    Msg = "''I see...''"
								MsgTimer = 70*5							
							Else
								Msg = "''Now I understand.''" ;Couldn't you reuse it? Like expand the player's backstory?								
								MsgTimer = 70*5
							EndIf								
							Case 1400.0
							If Juan = True Then
								Msg = "Thanks a lot, Juan." ;When I found out Juan posted this nonsense, I was angry but I didn't care so I followed through.
								MsgTimer = 70*5	
							EndIf 																
							Case 1500.0
							If Juan = True Then
								DeathMSG = "God DAMMIT, Juan. What were you thinking?"	;Mark says you shouldn't do it.	Now, apologise for your actions to Sam's work.
								Kill()
							    ;Juan = 0															
							Else
								Msg = "''I killed them.''" ;If you're seeing the comments from above, then that means it was typed by diszxchat. It's filled with garbage and lies.
								MsgTimer = 70*5
							EndIf
							Case 1650.0
							...
							EndIf																							
							Case 1750.0
							If Juan = False And (Not Condition113 = 1) Then							
								MSG = "''This is my punishment, that is why I'm here.''" ;I wanted to change but I must follow the original content.
								MsgTimer = 70*5
							ElseIf Juan = False And Condition113 = 1 Then
								MSG = "''Something's not right. I need... to...''" ;I wanted to change but I must follow the original content.
								MsgTimer = 70*5							    									
							EndIf								
							Case 2000.0
							If Juan = False And (Not Condition113 = 1) Then							
								Msg = "''Their lives are over, and it's all my fault.''" ;Vane Brain, PXLSHN, and me didn't have a problem with it. 
								MsgTimer = 70*5
							ElseIf Juan = False And Condition113 = 1 Then
								MSG = "''Right... It is my punishment after all.''"
								MsgTimer = 70*5							    									
							EndIf																
							Case 2050.0
							If Juan = False Then							
							...
							Case 2150.0
							If Juan = False And (Not Condition113 = 1) Then							
								Msg = "''I'm sorry.''" ;If PXLSHN ended up implementing this SCP as a hidden SCP, then lol.
								MsgTimer = 70*5
							ElseIf Juan = False And Condition113 = 1 Then
								MSG = "''Yet... I...''"
								MsgTimer = 70*5							    									
							EndIf
							Case 2350.0
							If Juan = False And Condition113 = 1 Then
							    PlaySound_Strict(NeckSnapSFX(Rand(0,2)))
							    KillTimer = Min(-1, KillTimer)
								DeathMSG = "Subject D-9341 was found dead near SCP-1074 testing room. The cause of death is a fatal cervical fracture, likely to be caused"
								DeathMSG = DeathMSG+" by SCP-173. Subject incinerated after the breach ended."
								Kill()	
							EndIf		                      
							Case 2500.0
							If Juan = False Then							
								Msg = "You feel weak." ;I don't feel weak. I'm very salty. That's the end of the story.
								MsgTimer = 70*5
								BlurTimer = 2500
							EndIf								
							Case 2600.0
			...
			Case "room012"
				...
				If PlayerRoom = e\room Then
					...
					Else
						...
						If Wearing714=False And WearingGasMask<3 And WearingHazmat<3 And SCP173Mode = False Then
							;temp = False
							...
							;DebugLog "WHERE IS IT?!"	
								...
								If dist < 0.6 Then
									...
									ElseIf e\EventState3>85*70 And e\EventState3-FPSfactor=<85*70
									    If Condition113 = 1 Then
										    DeathMSG = "Subject D-9341 found in a pool of blood next to SCP-012. Subject seems to have ripped open her wrists and written three extra "	
										Else																			
...
;						EndIf
			...
			Case "room079"
				...
				If PlayerRoom = e\room Then
					...
					If e\EventState = 0 Then
                        ...
                        ;Bloody Note Texture Randomisation
					    imgPath$ = "ProjectR\GFX\items\Passcode\Passcode"+Rand(0,10)+".jpg"
					
					    For itt.ItemTemplates = Each ItemTemplates
						      If itt\name = "Bloody Note" Then
							  If itt\img<>0 Then FreeImage itt\img	
							  itt\img = LoadImage_Strict(imgPath)
							  MaskImage(itt\img, 255,0,255)
							  itt\imgpath = imgPath
				...
				If PlayerRoom = e\room Then	
					...
					ElseIf e\EventState = 1
						...
						If dist<2.0 Then 
							...
							If (Not WearingHazmat) And Bloodloss>0.0 And SCP173Mode = False
								If Infect=0
									Infect=1
							...
							If e\EventState > 70 * 3 Then
								Select setting
									Case "rough"
										...
										If e\SoundCHN <> 0 Then StopChannel e\SoundCHN
										If Condition113 = 1 Then
										    DeathMSG = Chr(34)+"A heavily mutilated corpse found inside the output booth of SCP-914. DNA testing identified the corpse as Class D Subject D-9341 but under a opposite sex gender."
										    DeathMSG = DeathMSG + "The subject had obviously been "+Chr(34)+"refined"+Chr(34)+" by SCP-914 on the "+Chr(34)+"Rough"+Chr(34)+" setting, but we are still confused as to how she "
										    DeathMSG = DeathMSG + "ended up inside the intake booth and who or what wound the key, along with how his gender became the opposite."+Chr(34)
										Else										
						...
						For itt.ItemTemplates = Each ItemTemplates
							If IsItemGoodFor1162(itt) And Rand(6)=1
								...
								If Injuries > 15
									...
									If Condition113 = 1 Then
									    DeathMSG = DeathMSG + " An autopsy revealed that her right lung was missing, which suggests"
									Else
			...
			Case "newroom005"
				;[Block]
				If PlayerRoom = e\room Then
				    DebugLog "Entering the room..."
				    Local KeyDistance# = EntityDistance(Collider, e\room\Objects[0])
				    DebugLog KeyDistance
					;e\EventState = e\EventState2 + FPSfactor				
					If KeyDistance < 1.5 Then ;IfKey005 = True
						If (Not Contained106) And e\EventState2 = 0 Then
							d.Decals = CreateDecal(0, EntityX(e\room\Objects[1],True), EntityY(e\room\Objects[1],True), EntityZ(e\room\Objects[1],True), 0, e\room\angle + 360, Rand(360)) ;, 90, Rand(360), 0
							;de.Decals = CreateDecal(0, EntityX(e\room\Objects[1],True), -1531.0*RoomScale, EntityZ(e\room\Objects[1],True), 90, Rand(360), 0)

							d\Alpha = 0.01 : d\AlphaChange = 0.005
							d\Size = 0.1 : d\SizeChange = 0.003							
							;CreateNPC(NPCtypeOldMan, 0.0, 172.0, 1072.0)
								
							If Curr106\State > 0 Then ; And e\EventState > 50*8
							    ;Curr106\State = -11
							    ;Curr106\State = -0.1								
							    ;Curr106\Idle = True
					            ...
					            ShowEntity Curr106\obj
								e\EventState2 = 1					
					            ;PositionEntity(Curr106\obj, EntityX(Curr106\Collider), EntityY(Curr106\Collider), EntityZ(Curr106\Collider))																											
							    ;MoveEntity(Curr106\Collider, EntityX(e\room\Objects[3],True), EntityY(e\room\Objects[3],True), EntityZ(e\room\Objects[3],True))																						
							    ;PointEntity(Curr106\Collider, e\room\Objects[3])
							    ;AnimateNPC(Curr106, 284, 333, 0.02*35)															
								;e\EventState = e\EventState + FPSfactor
                                ;If EventState > 5*2 Then
								;	Curr106\State = -11
								;	Curr106\Idle = False									
								;EndIf	
							EndIf
			...
			Case "room009"
				...
				If e\Sound=0 Then LoadEventSound(e,"ProjectR\SFX\light_blink.ogg")
				...
				If e\EventState>=4290.0 And e\EventState3=1.0 Then
			        PlaySound_Strict LoadTempSound("ProjectR\SFX\ice-cracking.ogg")
					PlaySound TempSound
					e\EventState3=2.0
				EndIf
			...
			Case "room038"
				...
				Else
					e\room\RoomDoors[0]\locked = False					
				;If PlayerRoom = e\room Then	GiveAchievement(Achv038) :Having Remote Door off and giving you the achievement wouldn't make sense since you seen 038 through the glass.
				EndIf
				...
				If PlayerRoom = e\room Then
					...
					If e\EventState3=2 Then
						...
						If e\room\NPC[0] = Null Then
							...
							e\room\NPC[0]\State = 8
							e\room\NPC[0]\texture = "ProjectR\GFX\npcs\classd4.jpg"
							Local loadTex=LoadTexture("ProjectR\GFX\npcs\classd4.jpg")
							...
							Else
								...
								Select itemz\itemtemplate\tempname								
									...
									Case "scp178"																		
										...
										newTempName="glasses"									
									Case "scp1499"
										newName="Soviet GP-5 Gas Mask"
										newTempName="mask"
									Case "scp427"
										newName="Carved Locket"
										newTempName="locket"																										
								End Select
								...
								If (newName = "S-NAV Navigator Ultimate") Then
									cloneIt\state = 101
								EndIf
								If (newName = "Clipboard") Then								
									SetAnimTime cloneIt\model,17.0     								
								EndIf								
			...
			Case "room409"
				...
				If PlayerRoom = e\room Then
					If EntityY(Collider) < -2848*RoomScale Then
						...
						;If Music(25)=0 Then Music(25) = LoadSound_Strict("ProjectR\SFX\Music\SCP-409 Ambience.ogg") 
						ShouldPlay = 26						
					EndIf
					;If EntityY(Collider,True)<-4.0 Then
					...
					EndIf
					dist# = EntityDistance(Collider, e\room\Objects[1])
					If dist <1.0 And crystalization < 1 And WearingHazmat=False Then
						crystalization = 1
						If Rand(100) = 1 Then PlaySound_Strict(LoadTempSound("ProjectR\SFX\Organsm.ogg"))	
					EndIf
			...
			Case "room001" ;:P
				...
				If PlayerRoom = e\room ;Player is in the room + i + 
			       ;ENDSHN's "Inventory Condition" Met	
									
				    Local hasBloodyNote%=False
                    For i=0 To MaxItemAmount-1
                      If Inventory(i)<>Null Then
                          If Inventory(i)\itemtemplate\name="Bloody Note" Then
                              hasBloodyNote%=True
					...
					;DebugLog e\EventState2
					;DebugLog EntityDistance(e\room\Objects[0],Collider)

					If e\EventState2 > 0.9 And e\EventState2 < 79.6 Then
						Animate2(e\room\Objects[0], AnimTime(e\room\Objects[0]), 1, 353, 0.2, False)
						e\EventState2 = e\EventState2 + Min(FPSfactor,1.99)
					EndIf
					
					If EntityDistance(e\room\Objects[0],Collider)<2.00 Then
						BriefcaseOn = True																	
					   If EntityDistance(e\room\Objects[0],Collider)<0.75 Then
						   If e\EventState2 = 0.0 Then DrawHandIcon = True
						
						   If MouseHit1 And hasBloodyNote% = False Then
						       Msg = "You need a passcode."
						       MsgTimer = 70*5
						   ElseIf MouseHit1 And hasBloodyNote% = True Then						
						       Msg = "You unlocked the briefcase." ;It still unlocks regardless. But... Why? Hallucinations?
						       MsgTimer = 70*5	
                               e\EventState2 = 1.0										
						   EndIf
						
										
						;If BriefcaseOn = True Then
						;   AAText GraphicWidth/2, y+124*scale, BriefcaseInput,True,True
						;EndIf
                        ;If BriefcaseInput = "2003" Then
 						;    Msg = "Functional."
						;    MsgTimer = 70*5
						;    BriefcaseOn = False
						;Else 
                       ...
                       If e\EventState2 > 80.0 Then
                           GiveAchievement(Achv001)
                           PlaySound_Strict(LoadTempSound("ProjectR\SFX\Organsm.ogg"))
					       BriefcaseOn = False	
					       DebugLog "delete room001 event"
					       RemoveEvent(e)
				       EndIf
				...
				;[End Block]
			Case "room006" ;Modified Airlock with several things
				;[Block]
				;e\EventState: Determines if the airlock is in operation or not (
				;e\EventState2: The timer for the airlocks
				;e\EventState3: Checks if the player had left the airlock or not
				...
				x = UpdateLever(e\room\Levers[0]) ; Lever is switch on. x = 1 (Open) x = 0 (Closed)

				;e\Sound=LoadSound_Strict("SFX\Door\Airlock.ogg")	

				e\Sound=LoadSound_Strict("ProjectR\SFX\Incinerator Edited.ogg")	
																											
				If PlayerRoom=e\room Then
									
					e\EventState4 = UpdateElevators(e\EventState4, e\room\RoomDoors[3], e\room\RoomDoors[4],e\room\Objects[5],e\room\Objects[6], e)
					
				    If EntityY(Collider) < -2848*RoomScale Then 
				        GiveAchievement(Achv006)
								    
				        If ((EntityY(Collider)<=-15.8) And (WearingHazmat=False)) Then
						    ;Msg = "You feel powerful..." ;(Use it For debugging purposes)
						    ;MsgTimer = 70*8 ;(Use it For debugging purposes)
						    GodMode = True
				        EndIf												
					    If e\EventState = 0.0 And x = 1
						    If EntityDistance(e\room\Objects[0],Collider)<1.32 And e\EventState3 = 0.0 And x = 1																	
						        e\EventState = 1.0
							    StopChannel e\SoundCHN
							    e\SoundCHN = 0						
							    e\room\RoomDoors[1]\locked = False
						    ...
						    ElseIf EntityDistance(e\room\Objects[0],Collider)>2.4 And x = 1
							    e\EventState3 = 0.0
						    EndIf
					    Else
						    If e\EventState2 < 70*7 And x = 1
							    e\EventState2 = e\EventState2 + FPSfactor
							    e\room\RoomDoors[1]\open = False
							    e\room\RoomDoors[2]\open = False							
						        If e\EventState2 > 70*3 And e\EventState < 70*9 And GodMode = True And x = 1 And SCP173Mode = False ; And e\EventState < 70*5.5																																						
							        pvt% = CreatePivot(e\room\obj)								
							        For i = 0 To 11
									       If i = 0
									           ;PositionEntity pvt%,92.0,-44.0,848.0,False
							     ...
							     Next
							        ...
							        FreeEntity pvt
							    EndIf
                                If e\SoundChn=0 And GodMode = True And x = 1 And  e\EventState2 > 70*2.8 And SCP173Mode = False Then
                                    e\SoundCHN=PlaySound_Strict(e\Sound)
                                EndIf							
						        ;If e\EventState2 > 70*2.8 And e\EventState2 < 70*3 And GodMode = True And x = 1	Then e\SoundCHN=PlaySound_Strict(e\Sound)																																
							    If GodMode = True And e\EventState2 > 70*5 And x = 1 And SCP173Mode = False Then							
							        GodMode = False
						            Kill()
							        DeathMSG = "A pile of ashes were found in the incinerator from SCP-006's Containment Chamber."
						            DeathMSG = DeathMSG+"It was speculated that someone had come in contact with SCP-006 before he/she got incinerated to death."
				                    ;temp = 0						
						        EndIf
						    Else
							    e\EventState = 0.0
							    e\EventState2 = 0.0
							    e\EventState3 = 1.0
							    If e\room\RoomDoors[1]\open = False And GodMode = False
								    e\room\RoomDoors[1]\locked = False
								    e\room\RoomDoors[2]\locked = False
								    UseDoor(e\room\RoomDoors[1])
								    UseDoor(e\room\RoomDoors[2])
				...
				;If PlayerRoom=e\room Then
				;    If e\EventStr = "" Then 
				;        If ((EntityY(Collider)<=-0.2) And (WearingHazmat=False)) Then
				;		    e\EventStr = "Godess006"
				;		    e\EventState=e\EventState+FPSfactor
				;        Else
				;		    e\EventState=Max(e\EventState-(FPSfactor*0.5),0.0)
				;        EndIf
				;    EndIf
				;	If e\EventStr = "Godess006" And ((EntityY(Collider)<=-0.2) And (WearingHazmat=False)) Then
				;		Msg = "You feel powerful..."
				;		MsgTimer = 70*8
				;		GodMode = True
				;	EndIf						
				;EndIf	
				...
				;[End Block]
			Case "room1warehouse" ;Touching the SCP-113
				;[Block]
				;DebugLog e\EventState
				;DebugLog Repeat113	
				Local tempval# = e\EventState
											
				If PlayerRoom = e\room
                   If e\EventState2 > 0.5 Then
                       e\EventState2 = e\EventState2 + Min(FPSfactor,1.99)
                   EndIf
				   ...
				   ;Touching the Red Jasper									
                   If EntityDistance(e\room\Objects[0],Collider)<0.65 And e\EventState = 0
                       DrawHandIcon = True
					   If MouseHit1 Then
					      If e\EventState2 > 0.5 And e\EventState2 < 3000.0 Then	 			
						      Msg = "There's no reaction."
						      MsgTimer = 70*5		
						  ...
						  Else					      
						      e\EventState2 = 0					          
					          UnableToMove = True
						      Msg = "You feel a sharp pain in your body."
						      PlaySound_Strict(LoadTempSound("SFX\SCP\294\burn.ogg"))
						      MsgTimer = 70*5
						      e\EventState = 1.0
						  EndIf	   
                   ...
                   If e\EventState > 0.5 Then
                       e\EventState = e\EventState + FPSFactor
                       If tempval < 300.0 And e\EventState >= 300.0 Then
							BlurTimer = 50*70							
							Msg = "You're feeling dizzy."
							PlaySound_Strict(LoadTempSound("SFX\SCP\294\Vomit.ogg"))			
						    MsgTimer = 70*5							
					   ElseIf tempval < 1000.0 And e\EventState >= 1000.0 Then
						       ;BlurTimer = 10*70
							    Msg = "The pain is getting worse."
							    MsgTimer = 70*5							
					   ElseIf tempval < 3000.0 And e\EventState >= 3000.0 Then
							    Repeat113=Repeat113+1
							    UnableToMove = False
							    e\EventState2 = 1.0
							    e\EventState = 0.0	
							    ...
							    If Repeat113=3 Then
						            ...
						            Kill()
						            PlaySound_Strict(LoadTempSound("ProjectR\SFX\Organsm.ogg"))
						            DeathMSG = "Subject D-9341 was found dead with SCP-113 on his hands. Autopsy shows that Subject D-9341 had died due to a repeated exposure to SCP-113, causing damaged bones, organs, and tissues. His body was soon incinerated."
						            Repeat113 = 0
						           ;Condition113 = 0
						        Else
							        Msg = "You're freed from the stone."
							        MsgTimer = 70*5													
					            EndIf
					    EndIf	
                   EndIf
			...
			Case "room650"
				...
				If PlayerRoom = e\room Then 
                   If e\EventState = 0 Then	;--------Create NPC and be idle			
				        ;dist# = EntityDistance(Collider, e\room\obj)			
                        n.NPCs = CreateNPC(NPCtype650, EntityX(e\room\Objects[0],True), 0.5, EntityZ(e\room\Objects[0],True))
                        e\room\NPC[0]=n
                        e\room\NPC[0]\Idle = True			
				        PositionEntity e\room\NPC[0]\Collider, EntityX(e\room\Objects[0],True),EntityY(e\room\Objects[0],True)+0.4,EntityZ(e\room\Objects[0],True),True
						PointEntity e\room\NPC[0]\Collider, e\room\Objects[1]
						TurnEntity e\room\NPC[0]\Collider, 0, -129, 0					
				        ;RotateEntity Curr650\Collider, 0, EntityYaw(e\room\Objects[1], True), 0, True
				        e\EventState = 1
				   Else
				        If EntityDistance(Collider, e\room\Objects[0]) < 0.6 Then
				            e\room\NPC[0]\Idle = False
				            RemoveEvent(e)
				        EndIf
				        ;Not needed
			...
			Case "testroom173J"
				;[Block]
				If PlayerRoom = e\room	
					If Curr173J\Idle = 0 Then 
						If e\EventState = 0 Then
							If e\room\RoomDoors[0]\open = True
							PositionEntity(Curr173J\Collider, EntityX(e\room\Objects[0], True), 0.5, EntityZ(e\room\Objects[0], True))
							ResetEntity(Curr173J\Collider)
							e\EventState = 1
							EndIf
						Else
							If e\room\Objects[2]=0
							...
							ElseIf dist > 1.4
								;if the player moves a bit further and blinks, 173 attacks
								If e\EventState > 70*12 And BlinkTimer =< -10 Then
									If (EntityDistance(Curr173J\Collider, e\room\Objects[0]) > 5.0) Then
										;if 173 is far away from the room (perhaps because the player 
										;left and 173 moved to some other room?) -> disable the event
										RemoveEvent(e)	
									Else
										PlaySound2(LoadTempSound("SFX\General\GlassBreak.ogg"), Camera, Curr173J\obj) 
										FreeEntity(e\room\Objects[2])
										e\room\Objects[2]=0
										PositionEntity(Curr173J\Collider, EntityX(e\room\Objects[1], True), 0.5, EntityZ(e\room\Objects[1], True))
										ResetEntity(Curr173J\Collider)
										RemoveEvent(e)
									EndIf
								EndIf	
			...
			Case "room2doors173J"
				;[Block]
				If PlayerRoom = e\room Then
					If e\EventState = 0 And Curr173J\Idle = 0 Then
						If (Not EntityInView(Curr173J\obj, Camera)) Then
							e\EventState = 1
							PositionEntity(Curr173J\Collider, EntityX(e\room\Objects[0], True), 0.5, EntityZ(e\room\Objects[0], True))
							ResetEntity(Curr173J\Collider)
							RemoveEvent(e)
			...
			Case "room2pit173J"
				...
				If Curr173J\Idle = 0 Then 
					If e\room\dist < 8.0  And e\room\dist > 0 Then			
						If (Not EntityVisible(Curr173J\Collider, Camera)) And (Not EntityVisible(e\room\Objects[6], Camera)) Then 
							PositionEntity(Curr173J\Collider, EntityX(e\room\Objects[6], True), 0.5, EntityZ(e\room\Objects[6], True))
							ResetEntity(Curr173J\Collider)
							RemoveEvent(e)
						EndIf
					End If
				...
				If PlayerRoom = e\room Then
					...
					Else		
						...
						ElseIf e\EventState < 200
							...
							If e\EventState-FPSfactor =< 100 And e\EventState > 100 Then
								PlaySound_Strict LoadTempSound("SFX\ambient\general\ambient6.ogg")
								PositionEntity(Curr173J\Collider, EntityX(e\room\obj), 0.6, EntityZ(e\room\obj))
								ResetEntity(Curr173J\Collider)					
								Curr173J\Idle = True		
							EndIf
							LightBlink = 1.0
			...
			Case "gatea"
				...
				If PlayerRoom = e\room Then 
					...
					If e\EventState = 0 Then
						...
						For n.NPCs = Each NPCs
							If n <> Curr106 And n <> Curr173 And n <> Curr173J
								RemoveNPC(n)
							EndIf
						Next
						Curr173\Idle = True
