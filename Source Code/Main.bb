...
;    游戏基于SCP基金会社区的作品 (http://www.scp-wiki.net/).

;    源代码是根据知识共享署名共享3.0许可证授权的.
;    http://creativecommons.org/licenses/by-sa/3.0/

;    看 Credits.txt 查看贡献者列表
...
If FileSize("cpuid.dll")=0 Then InitErrorStr=InitErrorStr+ "FreeImage.dll"+Chr(13)+Chr(10)
If FileSize("fmod.dll")=0 Then InitErrorStr=InitErrorStr+ "FreeImage.dll"+Chr(13)+Chr(10)

If Len(InitErrorStr)>0 Then
	RuntimeError "缺少以下dll文件:"+Chr(13)+Chr(10)+Chr(13)+Chr(10)+InitErrorStr
EndIf
...
Global OptionFilePR$ = "ProjectR\options.ini"
...
;Global VersionNumber$ = ".0.4.0b"
Global VersionNumber$ = ".0.4.0a"
...
AppTitle "SCP:收容失效 - 重生计划 v"+VersionNumber
...
InitLoadingScreens("ProjectR\Loadingscreens\loadingscreens.ini")
...
Global BubbleFoam# ;BubbleTrigger#, Take1079#, Limit1079#
Global diceroll% = Rand(4)

Global BubbleTrigger%, Take1079%, Limit1079%
Global Wearing178%, WearingFake1499%
Global crystalization# ;BriefcaseOn#, Condition113#, Repeat113#

Global BriefcaseOn%, Condition113%, Repeat113%
Global Juan%
Global SCP173Mode%
		...
		If KeyHit(28) And ConsoleInput <> "" Then
			...
			Select Lower(StrTemp)
				Case "help"
					...
					Select Lower(StrTemp)
						Case "1",""
							...
							CreateConsoleMsg("- giveachievement")							
				...
				Case "status"
					...
					CreateConsoleMsg("BubbleFoam: "+BubbleFoam)
					CreateConsoleMsg("******************************")
					;[End Block]
				Case "camerapick"
					;[Block]
				...
				Case "kill","suicide"
					...
					Select Rand(4)
						...
						Case 2
							...
							If Condition113 = 1 Then
							    DeathMSG = DeathMSG + "The subject appears to have attained no physical damage, and there is no visible indication as to what killed her. "
							Else							
						...
						Case 4
							DeathMSG = "Subject D-9341 found dead in Sector [REDACTED]. "
							If Condition113 = 1 Then							
							    DeathMSG = DeathMSG + "The subject appears to have scribbled the letters "+Chr(34)+"kys"+Chr(34)+" in her own blood beside her. "
							Else	
...
;					EndIf
				...
				Case "juan"
					...
					End Select	
					If Juan Then
						CreateConsoleMsg("THE CODE DOESN'T WORK.")
					Else
						CreateConsoleMsg("GO SEE YOURSELF.")	
					EndIf					
					;[End Block]
				Case "reset372"
					;[Block]				
				...
				Case "debugcondition"
					...
					Select StrTemp
						...
						Case "off", "0", "false"
							Condition113 = 0
						Default
							Condition113 = Not Condition113
					End Select	
					If Condition113 Then
						CreateConsoleMsg("Testing.")
					Else
						CreateConsoleMsg("Test.")	
					EndIf					
					;[End Block]
				Case "troll"
					;[Block]
					TrollTex = Not TrollTex
					If TrollTex Then
						tex = LoadTexture_Strict("ProjectR\GFX\npcs\173o.pt", 1)					
						EntityTexture Curr173\obj, tex, 0, 0
						FreeTexture tex
						GiveAchievement(Achv173troll)
						CreateConsoleMsg("COMMERATE THE OLD TROLL.")
					Else
						tex2 = LoadTexture_Strict("GFX\npcs\173texture.jpg", 1)
						EntityTexture Curr173\obj, tex2, 0, 0
						FreeTexture tex2
						CreateConsoleMsg("WHY DO YOU HATE IT?")
					EndIf
					;[End Block]
				;Case "disable650"
					;[Block]
					;Curr650\Idle = 3
					;HideEntity Curr650\obj
					;HideEntity Curr650\Collider
					;[End Block]
				;Case "eanble650"
					;[Block]
					;Curr650\Idle = False
					;ShowEntity Curr650\obj
					;ShowEntity Curr650\Collider					
					;[End Block]
				Case "reset650"
					;[Block]				
					For n.NPCs = Each NPCs
				...
				Case "disable173J"
					;[Block]
					Curr173J\Idle = 3 ;This phenominal comment is brought to you by PolyFox. His absolute wisdom in this fatigue of knowledge brought about a new era of 173 state checks.
					HideEntity Curr173J\obj
					HideEntity Curr173J\Collider
					;[End Block]
				Case "enable173J"
					;[Block]
					Curr173J\Idle = False
					ShowEntity Curr173J\obj
					ShowEntity Curr173J\Collider
					;[End Block]
				Case "morph173j"
					;[Block]				
					BlinkTimer = -10
					...
					;	EndIf
					;Next
					ShowEntity(Overlay173)
					HideEntity(Fog)
					Injuries = 0
					Bloodloss = 0
					BlinkEffect = 0.0
					BlinkEffectTimer = 600000000000*999
					EyeStuck = 100000000*999
					Msg = "Your feel yourself like stone"
					MsgTimer = 70*7
					SCP173Mode=True
					SuperMan = True
					GodMode = True
				...
				Default
					;[Block]
					CreateConsoleMsg("指令无法找到.",255,0,0)
					;[End Block]
			End Select
...
Const PR_MusicFolder$ = "..\..\ProjectR\SFX\Music\" ;Thanks to PXLSHN
...
;Box Of Horrors Sounds

Music(26) = PR_MusicFolder+"SCP-409 Ambience" ;Sound Fix from PXLSHN
...
Global PrevInjuries#,PrevBloodloss#,PrevBubbleFoam#
...
Type Events
	...
	Field EventState#, EventState2#, EventState3#, EventState4# ;Use EventState4 if necessary for mods
...
Function InitEvents()
	...
	If Rand(5) < 5 Then 
		CreateEvent("testroom173J", "room2testroom2", 0, 1.0)
		CreateEvent("room2doors173J", "room2doors", 0, 0.5 + (0.4*SelectedDifficulty\aggressiveNPCs))
		CreateEvent("room2pit173J", "room2pit", 0, 0.4 + (0.4*SelectedDifficulty\aggressiveNPCs))
		CreateEvent("room2tunnel173J", "room2tunnel", 0)
	Else
    ...
    ;BoH/PR/BOH
    If diceroll = 1 Then CreateEvent("newroom005", "newroom005", 0)

	CreateEvent("room001", "room001", 0)
	CreateEvent("room006", "room006", 0)
	...
	CreateEvent("room1warehouse", "room1warehouse", 0)	
...
Global Overlay173tex%, Overlay173%
...
Repeat
	...
	Else
		...
		Else If SelectedDifficulty\saveType = SAVEONSCREENS And (SelectedScreen<>Null Or SelectedMonitor<>Null)
			If (Msg<>"游戏进度已保存." And Msg<>"无法保存在此位置."And Msg<>"此时无法保存.") Or MsgTimer<=0 Then
		...
		Else
			...
			If EndingTimer < -1000 And EndingTimer > -2000
				...
				If AchievementsMenu=0 Then 
					...
					Text x, y, "遇到SCP: " +scpsEncountered
					Text x, y+20*MenuScale, "成就解锁: " + achievementsUnlocked+"/"+(MAXACHIEVEMENTS)
					Text x, y+40*MenuScale, "找到的房间: " + roomsfound+"/"+roomamount
					Text x, y+60*MenuScale, "发现的文档: " +docsfound+"/"+docamount
					Text x, y+80*MenuScale, "中细化的项目SCP-914: " +RefinedItems			
...
Function InitCredits()
	...
	Local file% = OpenFile("ProjectR\PR Credits.txt")
	Local l$
...
Function MovePlayer()
	...
	If SuperMan Then
		...
		If SuperManTimer > 70 * 50 And SCP173Mode = False Then
	...
	If PlayerRoom\RoomTemplate\Name<>"pocketdimension" Then 
		If KeyDown(KEY_SPRINT) Then
			If Stamina < 5 Then
				temp = 0
				If WearingGasMask>0 Or Wearing1499>0 Or WearingFake1499>0 Then temp=1
	...
	If KeyHit(59) And SCP173Mode = True Then
		PositionEntity(Curr173\Collider,  EntityX(Collider),EntityY(Collider)+0.2,EntityZ(Collider))
		ResetEntity(Curr173\Collider)
	EndIf
	...
	If Injuries > 1.0 And BubbleTrigger = 0 Then
		temp2 = Bloodloss
		...
		EndIf
	ElseIf Injuries > 1.0 And BubbleTrigger = 1 Then 
		temp2 = BubbleFoam
		BlurTimer = Max(Max(Sin(MilliSecs2()/100.0)*BubbleFoam*30.0,BubbleFoam*2*(2.0-CrouchState)),BlurTimer)
		If (Not I_427\Using And I_427\Timer < 70*360) Then		
		    BubbleFoam = Min(BubbleFoam + (Min(Injuries,3.5)/300.0)*FPSfactor,100)
		EndIf
					
		If temp2 <= 60 And (Bloodloss > 60 Or BubbleFoam > 60) Then
	...
	If BubbleFoam > 0 Then
		If Rnd(200)<Min(Injuries,4.0) Then
			pvt = CreatePivot()
			PositionEntity pvt, EntityX(Collider)+Rnd(-0.05,0.05),EntityY(Collider)-0.05,EntityZ(Collider)+Rnd(-0.05,0.05)
			TurnEntity pvt, 90, 0, 0
			EntityPick(pvt,0.3)
			de.decals = CreateDecal(Rand(23,24), PickedX(), PickedY()+0.005, PickedZ(), 90, Rand(360), 0)
			de\size = Rnd(0.03,0.08)*Min(Injuries,3.0) : EntityAlpha(de\obj, 1.0) : ScaleSprite de\obj, de\size, de\size
			tempchn% = PlaySound_Strict (SizzSFX(Rand(0,2)))
			ChannelVolume tempchn, Rnd(0.0,0.8)*SFXVolume
			ChannelPitch tempchn, Rand(20000,30000)
		...
		If BubbleFoam > 60 Then Crouch = True
		If BubbleFoam => 100 Then 
			Kill()
			HeartBeatVolume = 0.0
		ElseIf BubbleFoam > 80.0
			HeartBeatRate = Max(150-(BubbleFoam-80)*5,HeartBeatRate)
			HeartBeatVolume = Max(HeartBeatVolume, 0.75+(BubbleFoam-80.0)*0.0125)	
		ElseIf BubbleFoam > 35.0
			HeartBeatRate = Max(70+BubbleFoam,HeartBeatRate)
			HeartBeatVolume = Max(HeartBeatVolume, (BubbleFoam-35.0)/60.0)			
		EndIf
	EndIf
	...
	If HealTimer > 0 Then
		...
		BubbleFoam = Min(BubbleFoam + (2 / 400.0) * FPSfactor, 100)		
	EndIf
...
Function MouseLook()
	...
	If WearingGasMask Or WearingHazmat Or Wearing1499 Or WearingFake1499 Then
    ...
    ;Box Of Horrors/Main Game
	If Wearing178=1 Then ;>0
		ShouldPlay = 14
		ShowEntity(GlassesOverlay)
	ElseIf Wearing178=2 Then
		ShowEntity(GlassesOverlay)		
	Else
	...
	For i = 0 To 5
		If SCP1025state[i]>0 And SCP173Mode = False Then
			Select i
				Case 0 ;common cold
					If FPSfactor>0 Then 
						If Rand(1000)=1 Then
				...
				Case 5;cardiac arrest
					...
					If SCP1025state[i]>110 Then
						...
						If SCP1025state[i]>140 Then
							If Condition113 = 1 Then		 
							    DeathMSG = Chr(34)+"She died of a cardiac arrest after reading SCP-1025, that's for sure. Is there such a thing as psychosomatic cardiac arrest, or does SCP-1025 have some "
							Else
...
Function DrawGUI()
	...
	If ClosestItem <> Null Then
		...
		;If BriefcaseOn = False Then DrawImage(HandIcon2, GraphicWidth / 2 + Sin(yawvalue) * (GraphicWidth / 3) - 32, GraphicHeight / 2 - Sin(pitchvalue) * (GraphicHeight / 3) - 32)
		
		If PlayerRoom\RoomTemplate\Name = "room001" And ClosestItem\itemtemplate\tempname = "scp001" And BriefcaseOn = True Then 
	...
	If HUDenabled Then 
		...
		If DebugHUD Then
			...
			Text x - 50, 410, "BubbleFoam: " + BubbleFoam			
			If Curr173 <> Null
			...
			For npc.NPCs = Each NPCs
				...
				If npc\NPCtype = NPCtypeMTF Then
					Text x - 50, 630 + 60 * offset, "MTF " + offset + " Position: (" + f2s(EntityX(npc\obj), 3) + ", " + f2s(EntityY(npc\obj), 3) + ", " + f2s(EntityZ(npc\obj), 3) + ")"
					Text x - 50, 650 + 60 * offset, "MTF " + offset + " State: " + npc\State
					Text x - 50, 670 + 60 * offset, "MTF " + offset + " LastSeen: " + npc\lastseen					
					offset = offset + 1
				EndIf
			Next
			If PlayerRoom\RoomTemplate\Name$ = "dimension1499"
			...
			Text x + 350, 190, "SCP-409 crystalization: "+crystalization			
			For i = 0 To 5
				Text x + 350, 210+(20*i), "SCP-1025 State "+i+": "+SCP1025state[i]
			Next
			If SelectedMonitor <> Null Then
				Text x + 350, 330, "Current monitor: "+SelectedMonitor\ScrObj
			Else
				Text x + 350, 330, "Current monitor: NULL"
			EndIf
	...
	;-------------------------- END --------------------------			
		...
		If shouldDrawHUD Then
			...
			If KeypadMSG <> "" Then 
				...
				EndIf
			Else
				Text GraphicWidth/2, y+70*scale, "访问代码: ",True,True	
				SetFont Font4
		...
		For  n% = 0 To MaxItemAmount - 1
			...
			If Inventory(n) <> Null Then
				...
				Select Inventory(n)\itemtemplate\tempname 
					...
					Case "mask"
						If WearingFake1499=1 Then Rect(x - 3, y - 3, width + 6, height + 6)							
				End Select
			EndIf
		...
		If SelectedItem <> Null Then
			Select SelectedItem\itemtemplate\tempname
				...
				Case "battery"
					...
					;InvOpen = True
					;[End Block]
				Case "key1", "key2", "key3", "key4", "key5", "key6", "keyomni", "scp860", "hand", "hand2", "25ct", "scp005" ;BoH
					;[Block]
					...
					If CanUseItem(False, False, True)
						Select Rand(5)
							...
							Case 3
								...
								BubbleFoam = Max(0, BubbleFoam - Rnd(10,100))
						...
						If CanUseItem(False, True, True)
							...
							If SelectedItem\state = 100 Then
								...
								Else
									...
									BubbleFoam = Max(0, BubbleFoam - Rand(10,20))
				...
				Case "eyedrops"
					...
					If CanUseItem(False,False,False)
						If (Not (Wearing714=1)) And SCP173Mode = False Then ;wtf is this
							BlinkEffect = 0.6
							BlinkEffectTimer = Rand(20,30)
							BlurTimer = 200
						EndIf
				...
				Case "fineeyedrops"
					;[Block]
					If CanUseItem(False,False,False)
						If (Not (Wearing714=1)) And SCP173Mode = False Then 
							BlinkEffect = 0.4
				...
				Case "cup"
					...
					If CanUseItem(False,False,True)
					    ...
					    If SelectedItem\state2=1 Then iniStr = "ProjectR\Data\SCP-294.ini"
						...
						BubbleFoam = Max(BubbleFoam + GetINIInt2(iniStr, loc, "bubble foam"),0);*temp
                        ...
                        ;Crystal
						If GetINIInt2(iniStr, loc, "crystalization") Then crystalization = 1
				...
				Case "navigator", "nav"
					...
					If (Not NavWorks) Then
						If (MilliSecs2() Mod 1000) > 300 Then
							...
							Text(x, y + height / 2 - 60, "位置未知", True)						
						EndIf
					Else
						...
						If SelectedItem\state > 0 And (Rnd(CoffinDistance + 15.0) > 1.0 Or PlayerRoom\RoomTemplate\Name <> "coffin") Then
							...
							If (MilliSecs2() Mod 1000) > 300 Then
								If SelectedItem\itemtemplate\name <> "S-NAV 310 Navigator" And SelectedItem\itemtemplate\name <> "S-NAV Navigator Ultimate" Then
									Text(x - width/2 + 10, y - height/2 + 10, "地图数据库脱机")
								EndIf
									...
									If np\NPCtype = NPCtype650
										...
										If dist < 8.0 * 4 Then
											If (Not np\HideFromNVG) Then
												Color 100, 0, 0
												Oval(x - dist * 1.5, y - 7 - dist * 1.5, dist * 3, dist * 3, False)
												Text(x - width / 2 + 10, y - height / 2 + 30 + (20*SCPs_found), "SCP-650")
												SCPs_found% = SCPs_found% + 1
											EndIf
										EndIf
										Exit
				...
				Case "scp500death"
					...
					;[End Block]
				;Box Of Horrors	
				Case "clonedpill" ;Failed Clone of SCP-500
					;[Block]				
				...
				EndIf														
					;[End Block]
				Case "book" ;Failed Clone of SCP-1025
					;[Block]					
					;GiveAchievement(Achv1025) 
					If SelectedItem\itemtemplate\img=0 Then
						SelectedItem\state = Rand(0,5)
					...
					;[End Block]
				Case "cowbell" ;Failed Clone of SCP-513
					;[Block]
					PlaySound_Strict LoadTempSound("SFX\SCP\513\Bell1.ogg")
						
					SelectedItem = Null
					;[End Block]					
				Case "glasses" ;Failed Clone of SCP-178
					;[Block]				
					...
					SelectedItem = Null	
					;[End Block]	
				Case "mask" ;Failed Clone of SCP-1499
					;[Block]
					If WearingHazmat>0
						Msg = "你不能同时戴防毒面具和防护服."
						MsgTimer = 70 * 5
				...
				Case "scp001"
					...
					Select Rand(10)
						...
						Case 3 ;SCP-080 
							...
							;Msg = "Sorry, the option you chose is not available. We are currently renovating the old option."							
							;MsgTimer = 70*7	
							
						    ;-------------- SCP-080 Spawn (NEW) --------------
							If Curr080 = Null
						        Curr080 = CreateNPC(NPCtype080, EntityX(Collider),EntityY(Collider) + 0.2,EntityZ(Collider))	
						    Else
			                    PositionEntity Curr080\Collider, EntityX(Collider),EntityY(Collider) + 0.2,EntityZ(Collider)	
								ResetEntity(Curr080\Collider)
					        EndIf
						    ;-------------- END --------------																		
						Case 4 ;SCP-1025
							If SelectedItem\itemtemplate\img=0 Then
						...
						Case 6 ;SCP-008																											
							Infect=1
							Msg = "你感觉有什么东西在你的血管里流下来."							
							MsgTimer = 70*7							
						Case 7 ;SCP-173
				            ;If Curr173\Idle = 0 Then
								;PositionEntity Curr173\Collider, EntityX(Collider),EntityY(Collider)+0.2,EntityZ(Collider)
							;	Curr173 = CreateNPC(NPCtype173, EntityX(Collider),EntityY(Collider)+0.2,EntityZ(Collider))
							;    ResetEntity(Curr173\Collider)
							;Else
							;	PositionEntity Curr173\Collider, EntityX(Collider),EntityY(Collider)+0.2,EntityZ(Collider)							
                            ;EndIf
						    ...
						    If (Not Curr173\Idle = 3) Then
						      ...
						      EndIf	
							EndIf                
						Case 8 ;SCP-106 (Except he spawns)
				            ;If Curr106\Idle = 0 Then
							;	PositionEntity Curr106\Collider, EntityX(Collider),EntityY(Collider)+0.2,EntityZ(Collider)
								;Curr106 = CreateNPC(NPCtype106, EntityX(Collider),EntityY(Collider) - 0.5,EntityZ(Collider))
							;	n\State = -1
							;    ResetEntity(Curr106\Collider)
							;Else
							;	PositionEntity Curr106\Collider, EntityX(Collider),EntityY(Collider) - 0.5,EntityZ(Collider)							
                            ;EndIf

                            ;Done - Problem
						    If Contained106 = False	Then				
						       If Curr106 = Null Then																								
							       Curr106 = CreateNPC(NPCtypeOldMan, EntityX(Collider),EntityY(Collider) - 0.5,EntityZ(Collider))
							       Curr106\State = -1
							   Else							
			                       PositionEntity Curr106\Collider, EntityX(Collider),EntityY(Collider) - 0.5,EntityZ(Collider)
			                       ResetEntity(Curr106\Collider)
							...
							EndIf			
						Case 9 ;SCP-096
				           ; If Curr096\Idle = 0 Then
								;PositionEntity Curr096\Collider, EntityX(Collider),EntityY(Collider)+0.2,EntityZ(Collider)
                            ;    Curr096 = CreateNPC(NPCtype096, EntityX(Collider),EntityY(Collider)+0.2,EntityZ(Collider))								
							...
							EndIf				
						Case 10 ;SCP-066
							;PositionEntity Curr066\Collider,x,y,z, True
							
                            ;Done							
							If Curr066 = Null Then
						        Curr066 = CreateNPC(NPCtype066, EntityX(Collider),EntityY(Collider) + 0.2,EntityZ(Collider))	
						    Else
			                    PositionEntity Curr066\Collider, EntityX(Collider),EntityY(Collider) + 0.2,EntityZ(Collider)	
				...
				Case "scp1079sweet"
				    ...
				    If Take1079 > 4 Then
					    ...
					    DeathMSG = "九尾狐发现D级人员D-9341死亡,尸体周围覆盖着粉红色泡泡糖香味的泡沫."
					    DeathMSG = DeathMSG+"有毒告表明,Wondertainment博士的泡泡糖Bonbons中含有泡泡糖香味的泡沫!???,"
				...
				Case "scp1079"
                    ...
                    ;it = CreateItem("SCP-1079-01", "scp1079sweet", 1, 1, 1)
					;it\Picked = True
					;it\Dropped = -1
					;it\itemtemplate\found=True
			        ;EntityType (it\collider, HIT_ITEM)
					...
					;If ItemAmount < MaxItemAmount Then ;Temporary - Need further test
	...
	If MenuOpen Then
		...
		If PlayerRoom\RoomTemplate\Name$ <> "exit1" And PlayerRoom\RoomTemplate\Name$ <> "gatea"
			If StopHidingTimer = 0 Then
				If EntityDistance(Curr173\Collider, Collider)<4.0 Or EntityDistance(Curr106\Collider, Collider)<4.0 Or EntityDistance(Curr173J\Collider, Collider)<4.0 Then 
					StopHidingTimer = 1
				EndIf	
		...
		ElseIf KillTimer >= 0 Then
			SetFont Font2
			Text(x, y-(122-45)*MenuScale, "暂停",False,True)
			SetFont Font1
		Else
			SetFont Font2
		...
		If AchievementsMenu <= 0 And OptionsMenu <= 0 And QuitMSG <= 0
			...
			Text x, y+20*MenuScale, "保存: "+CurrSave
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu > 0 And QuitMSG <= 0 And KillTimer >= 0
			...
			If DrawButton(x-5*MenuScale,y,100*MenuScale,30*MenuScale,"图像",False) Then OptionsMenu = 1
			...
			If DrawButton(x+325*MenuScale,y,100*MenuScale,30*MenuScale,"成就",False) Then OptionsMenu = 4
			...
			Select OptionsMenu
				...
				Case 2 ;Audio
					...
					If EnableUserTracks
						...
						If UserTrackMode
							Text x, y + 20 + 3 * MenuScale, "重复"
						Else
							Text x, y + 20 + 3 * MenuScale, "随机的"
						EndIf
						If MouseOn(x+270*MenuScale,y+MenuScale,20*MenuScale,20*MenuScale)
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu <= 0 And QuitMSG > 0 And KillTimer >= 0
			...
			If SelectedDifficulty\saveType = SAVEONQUIT Or SelectedDifficulty\saveType = SAVEANYWHERE Then
				...
				If AbleToSave
					QuitButton = 140
					If DrawButton(x, y + 60*MenuScale, 390*MenuScale, 60*MenuScale, "保存 & 退出") Then
						DropSpeed = 0
						SaveGame(SavePath + CurrSave + "\")
						NullGame()
						MenuOpen = False
		...
		If AchievementsMenu<=0 And OptionsMenu<=0 And QuitMSG<=0 Then
			...
			Else
				...
				EndIf
				If DrawButton(x, y + 80*MenuScale, 390*MenuScale, 60*MenuScale, "退出到菜单") Then
					NullGame()
					MenuOpen = False
					MainMenuOpen = True
					MainMenuTab = 0
...
Function LoadEntities()
    ...
    ;BoH
	BinocularsTexture = LoadTexture_Strict("ProjectR\GFX\BinocularsOverlay.jpg", 1)
	...
	IceTexture = LoadTexture_Strict("ProjectR\GFX\map\red_ice.png",1)
	...
	CrystalTexture = LoadTexture_Strict("ProjectR\GFX\CrystalOverlay.png",1)
	...
	;SCP-178	
	GlassesTexture = LoadTexture_Strict("ProjectR\GFX\GlassesOverlay.jpg",1)
	...
	Overlay173tex = LoadTexture_Strict("ProjectR\GFX\173Overlay.jpg", 1)
	Overlay173 = CreateSprite(ark_blur_cam)
	ScaleSprite(Overlay173, Max(GraphicWidth / 1024.0, 1.0), Max(GraphicHeight / 1024.0 * 0.8, 0.8))
	EntityTexture(Overlay173, Overlay173tex)
	EntityBlend (Overlay173, 2)
	EntityFX(Overlay173, 1)
	EntityOrder Overlay173, -1003
	MoveEntity(Overlay173, 0, 0, 1.0)
	HideEntity(Overlay173)
...
;	HideEntity OBJTunnel(6)
    ...
    ;BoH
	DecalTextures(21) = LoadTexture_Strict("ProjectR\GFX\decal20.png", 1 + 2)
	DecalTextures(22) = LoadTexture_Strict("ProjectR\GFX\decalfungus.png", 1 + 2)

    ;PR
	For i = 23 To 24
		DecalTextures(i) = LoadTexture_Strict("ProjectR\GFX\bubblescentdrop"+(i-22)+".png", 1 + 2)	
	Next
...
Function InitNewGame()
	...
	Curr173J = CreateNPC(NPCtype173J, 0, -30.0, 0)	
    ...
    diceroll = Rand(4)
	If (Rand(0,100)<2) Then Juan = True ; Snicker
...
Function NullGame(playbuttonsfx%=True, nomenuload%=False)
	...
	Local PlayerRoomZone = PlayerZone
	...
	If Curr173J <> Null Then Curr173J\Idle = False
	...
	If (Not nomenuload)
		...
		;If roomlocationentry = "" Then
		;	roomlocationentry = PlayerRoomName
		;EndIf
		;Init3DMenu(entry)

		PutINIValue("ProjectR\options.ini","options","game progress",roomlocationentry)								
	EndIf
...
Function Use914(item.Items, setting$, x#, y#, z#)
	...
	Select item\itemtemplate\name
		...
		Case "Soviet GP-5 Gas Mask" ;Project Resurrection
				Select setting
				Case "rough", "coarse"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.005, z, 90, Rand(360), 0)
					d\Size = 0.12 : ScaleSprite(d\obj, d\Size, d\Size)
	...
	If temp Then
		If MouseHit1 Then
			...
			If temp And Input294<>"" Then ;dispense
				...
				If Input294<>""
				    Local isPR% = 0 ;Ensures to look into ProjectR
				    Local drinksList$ = "DATA\SCP-294.ini" ;SCP CB 				
					Local loc% = GetINISectionLocation("DATA\SCP-294.ini",Input294)						
				EndIf
				
                ;If not found, tries to find specific drink in ProjectR folder
				If loc<=0 Then loc% = GetINISectionLocation("ProjectR\Data\SCP-294.ini", Input294) : isPR = 1 : drinksList="ProjectR\Data\SCP-294.ini"
								
				If loc > 0 Then
					;strtemp$ = GetINIString2("DATA\SCP-294.ini", loc, "dispensesound")
					strtemp$ = GetINIString2(drinksList, loc, "dispensesound")
					...
					If GetINIInt2("DATA\SCP-294.ini", loc, "explosion")=True Then 
						...
						DeathMSG = GetINIString2(drinksList, loc, "deathmessage")						
					EndIf
					
					;strtemp$ = GetINIString2("DATA\SCP-294.ini", loc, "color")
					strtemp$ = GetINIString2(drinksList, loc, "color")
					...
					alpha# = Float(GetINIString2(drinksList, loc, "alpha"))
					glow = GetINIInt2(drinksList, loc, "glow")
...
Function Use427()
	...
	If I_427\Timer < 70*360
		If I_427\Using=True And (Not I_427\Using=3) Then
			...
			EndIf
			If crystalization > 0.0 Then
				crystalization = Max(crystalization - 0.001 * FPSfactor,0.0)
			EndIf			
			For i = 0 To 5
	...
	Else
		...
		If I_427\Timer >= 70*420 Then
			Kill()
			DeathMSG = Chr(34)+"请求MTF Nu-7 落锤 的支持.我们需要更多的火力来摧毁这东西."+Chr(34)
...
Function Update409()
	...
	If crystalization>0 Then
		...
		ElseIf crystalization > 94.05 And crystalization < 95.26 Then ;94 and 95.2
			crystalization = Min(crystalization+FPSfactor*0.004,100)
			Playable=False
			CanBreathe=False
			BlurTimer=4.0
		...
		If crystalization > 93 And temp =< 93 Then
			PlaySound_Strict LoadTempSound("ProjectR\SFX\409playerdeath.ogg")
			PlaySound TempSound
			Injuries = Max(Injuries,2.0)
		EndIf
		
		If crystalization >= 95.26 Then ;95.2
			testertesttest = True
		EndIf
		...
		If (testertesttest = True) And (KillTimer>=0) Then	
			Playable=True
			CanBreathe=True
			DeathMSG = "发现了一堆SCP-409晶体,通过比较死者名单,发现D-9341与SCP-409有过身体接触. "
			DeathMSG = DeathMSG + "遗体和设施中水晶泛滥的区域一起被焚烧."
			Kill()
		EndIf
	Else
		HideEntity CrystalOverlay	
...
Function TakeOffStuff(flag%=0)
	...
	If Right(numb_flag%,1) = 1
		...
		WearingFake1499 = False		
		DebugLog "GasMask and Soviet GP-5 GasMask Off"
	EndIf
	If Len(numb_flag%)>1
...
Function RenderWorld2()
	...
	If BlinkTimer < - 16 Or BlinkTimer > - 6
		If WearingNightVision=2 And hasBattery<>0 Then ;show a HUD
			...
			Text GraphicWidth/2,(20+plusY)*MenuScale,"刷新中的数据",True,False
			
			Text GraphicWidth/2,(60+plusY)*MenuScale,Max(f2s(NVTimer/60.0,1),0.0),True,False
			Text GraphicWidth/2,(100+plusY)*MenuScale,"秒",True,False
	...
	If BlinkTimer < - 16 Or BlinkTimer > - 6
		If (WearingNightVision=1 Or WearingNightVision=2) And (hasBattery=1) And ((MilliSecs2() Mod 800) < 400) Then
			...
			Text GraphicWidth/2,20*MenuScale,"警告: 电池电量低",True,False
			Color 255,255,255
		EndIf
	EndIf
End Function
...
Function IsItemGoodFor1162(itt.ItemTemplates)
	...
	Select itt\tempname
		...
		Case "misc", "420", "cigarette"
			Return True
		Case "vest", "finevest","gasmask","mask"
			Return True
		Case "radio","18vradio"
			Return True
		...
		Case "bloody note"
			If itt\img<>0 Then FreeImage itt\img	
			itt\img = LoadImage_Strict("ProjectR\GFX\items\Passcode\Passcode"+Rand(0,10)+".jpg") ;Gives a random bloody note.
			Return True				
		Default
			If itt\tempname <> "paper" Then
				Return False
...
Function CatchErrors(location$)
         InitErrorMsgs(6)
         SetErrorMsg(0, "一个错误发生在 SCP - 收容失效: 重生计划 v" + VersionNumber)
         SetErrorMsg(1, "地图种子: " + RandomSeed)
         SetErrorMsg(2, "存档时间: " + CurrentDate() + " at " + CurrentTime() + Chr(10) + "OS: " + SystemProperty("os") + " " + (32 + (GetEnv("ProgramFiles(X86)") <> 0) * 32) + " bit (Build: " + SystemProperty("osbuild") + ")" + Chr(10))
         SetErrorMsg(3, "游戏内存: " + ((TotalVidMem() / 1024) - (AvailVidMem() / 1024)) + " MB/" + (TotalVidMem() / 1024) + " MB" + Chr(10))
         SetErrorMsg(4, "总内存: " + ((TotalPhys() / 1024) - (AvailPhys() / 1024)) + " MB/" + (TotalPhys() / 1024) + " MB" + Chr(10))
         SetErrorMsg(5, "错误发生在: " + Location + Chr(10) + Chr(10) + "请带着截图来联系我们!") 
End Function
...
Function CanUseItem(canUseWithHazmat%, canUseWithGasMask%, canUseWithEyewear%)
	If (canUseWithHazmat = False And WearingHazmat) Then
		...
		MsgTimer = 70*5
		Return False
	Else If (canUseWithGasMask = False And (WearingGasMask Or Wearing1499 Or WearingFake1499))
		Msg = "你不能在戴防毒面具时使用该物品."
		MsgTimer = 70*5
		Return False
