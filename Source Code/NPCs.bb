...
Global Curr066.NPCs, Curr080.NPCs, Curr173J.NPCs

Const NPCtype178% = 23, NPCtype080% = 24, NPCtype650% = 25, NPCtype173J% = 26, NPCtypeIkobot% = 27
;[End Block]
...
Function CreateNPC.NPCs(NPCtype%, x#, y#, z#)
	...
	Select NPCtype
		...
		Case NPCtypeClerk
			...
			;[End Block]
		;----------------- PR -----------------
		Case NPCtype080 ;Sprite Entity 
			;[Block]
			n\NVName = "SCP-080"
			n\Collider = CreatePivot()
			EntityRadius n\Collider, 0.2
			EntityType n\Collider, HIT_PLAYER	
			n\Speed = (GetINIFloat("ProjectR\DATA\NPCs.ini", "SCP-080", "speed") / 100.0)

			n\obj = LoadSprite("ProjectR\GFX\npcs\darkformeyes2.jpg", 1+2)				
			ScaleSprite(n\obj, 0.08, 0.08)
			EntityBlend (n\obj, 3)
			EntityFX(n\obj, 1 + 8)
			SpriteViewMode(n\obj, 2) ;2
			;PositionEntity n\obj,0,0.80,0
			;RotateEntity n\obj,0,180,0

			;n\obj = LoadSprite("GFX\npcs\oldmaneyes.jpg", 1+2)			
			;n\obj = LoadAnimMesh_Strict("GFX\npcs\106_2.b3d")												
			;temp# = (GetINIFloat("DATA\NPCs.ini", "SCP-106", "scale") / 2.2)		
			;ScaleEntity n\obj, temp, temp, temp					
			...
			;MoveEntity(n\obj2, 0, 3.6 * 0.11, -1.5 * 0.11)

						
			;n\obj = CreateSprite()
			;EntityTexture(n\obj, OldManEyes)
		...
		Case NPCtype178
			...
			If n\obj = 0 Then 
				n\obj = LoadAnimMesh_Strict("ProjectR\GFX\npcs\npc178.b3d")
			EndIf
		...
		Case NPCtype650
			...
			temp# = (GetINIFloat("ProjectR\DATA\NPCs.ini", "SCP-650", "scale") / MeshDepth(n\obj))			
			ScaleEntity n\obj, temp,temp,temp
			
			n\Speed = (GetINIFloat("ProjectR\DATA\NPCs.ini", "SCP-650", "speed") / 100.0)
		...
		Case NPCtype173J
			...
			n\obj = LoadMesh_Strict("ProjectR\GFX\npcs\173j.b3d")
		...
		Case NPCtypeIkobot
			...
			n\Speed = (GetINIFloat("DATA\NPCs.ini", "Guard", "speed") / 100.0)
			temp# = (GetINIFloat("DATA\NPCs.ini", "Guard", "scale") / 3.91)
...
;								EndIf
			...
			Case NPCtype080 ;------------------------------------------------------------------------------------------------------------------
				...
				dist = EntityDistance(n\obj, Collider)
				;temp% = False
				DebugLog dist
																
				Select n\State									
					Case 1 ;Close to 080 -> 080 looks at it -> staring contest = dies (WHO WOULD WANT TO STARE FOR 30 MINUTES?)
					;[Block]
						If dist < 1.5 Then ; Closer = 080 looks at the player ; 1.8
						    PointEntity n\obj, Collider						    
						    RotateEntity n\Collider, 0, CurveAngle(EntityYaw(n\obj), EntityYaw(n\Collider), 10.0), 0
							;n\State3 = n\State3 + FPSfactor
						    If (Not EntityInView(n\obj, Camera)) Then ;If the object not in view = Reset timer						
							     n\State3 = 0							 
							Else
							     n\State3 = n\State3 + FPSfactor
							EndIf							
						Else
                            RotateEntity n\obj,0,EntityYaw(n\obj)+180,0						
							n\State = 0
							n\State3 = 0
						EndIf							
						
						If n\State3 > 75*9 And (Not GodMode) Then ; 1800*0 = 30 Minutes
						 Kill()
						 DeathMSG = "Subject D-9341 was found asleep in the facility."
						 If Condition113 = 1 Then
						 DeathMSG = DeathMSG+" After the subject woke up, he was experiencing fear from what it is assume as a psychological trauma. The subject was then terminated."
						 Else
						 DeathMSG = DeathMSG+" After the subject woke up, the subject was unresponsive and in fear. She was relocated to a facility for further treatment and test."	
						 EndIf				
				        EndIf							
					;[End Block]
					Case 2 ; SCP-080's Containment Chamber State W.I.P (Maybe)
					;[Block]
					;[End Block]										
					Default ; Idle, Wondering around (SCP-1499 code pasted)
					;[Block]
					   ;n\CurrSpeed = CurveValue(n\Speed, n\CurrSpeed, 5.0)
					   If dist < 1.8 Then ;1.5
					       n\State = 1
					       n\CurrSpeed = 0.0
					   Else
					       n\State = 0
				...
				;SCP-080's Particles
				p.Particles = CreateParticle(EntityX(n\Collider,True),EntityY(n\Collider,True),EntityZ(n\Collider,True),0,Rnd(0.2,0.3),-0.2,300)
			    p\A = 0.45 ; 0.25
			    p\SizeChange = 0.0098 ;Rnd(0.0,0.005)
			    p\Achange = Rnd(-0.0025,-0.003)
			    p\speed = 0.008
			    RotateEntity p\pvt, 90, 0, 0			
			    ;p\Achange = -0.005			
			    ;TurnEntity p\pvt,0,EntityYaw(n\Collider,True)+180,0
			    
			    ;Positioning object
			    PositionEntity(n\obj, EntityX(n\Collider), EntityY(n\Collider) + 0.64, EntityZ(n\Collider))
			    RotateEntity n\obj, EntityPitch(n\Collider), EntityYaw(n\Collider) + 180, 0
			    ;RotateEntity(n\obj, 0, EntityYaw(n\Collider) - 180, 0)			
				;[End Block]
			Case NPCtype650
				;[Block]
				...
				DebugLog dist#							
                    ...
                    ;Player blinks -> 650 teleport behind the player (Issue fixed)
					      ...
					      MoveEntity(n\Collider, 0, 0, +0.95) ;1.5
					      ResetEntity n\Collider
											...
											For w.waypoints = Each WayPoints
												If w\door=Null And Rand(5)=1 Then
													...
													If x < 25.0 And x > 15.0 Then
														z = Abs(EntityZ(Collider)-EntityZ(w\obj,True))
														If z < 25 And z > 15.0 Then
															DebugLog "MOVING 173-J TO "+w\room\roomtemplate\name
															PositionEntity n\Collider, EntityX(w\obj,True), EntityY(w\obj,True)+0.25,EntityZ(w\obj,True)
															ResetEntity n\Collider
															Exit
														EndIf
									...
									If temp Then 				
										If dist < 0.65 Then
											If KillTimer >= 0 And (Not GodMode) Then
												...
												Select PlayerRoom\RoomTemplate\Name
													Case "lockroom", "room2closets", "coffin"
														DeathMSG = "Subject D-9341. Cause of death: Fatal cervical fracture. The surveillance tapes confirm that the subject was killed by SCP-173-J."	
													;Case "173"
													;	DeathMSG = "Subject D-9341. Cause of death: Fatal cervical fracture. According to Security Chief Franklin who was present at SCP-173-J's containment "
													;	DeathMSG = DeathMSG + "chamber during the breach, the subject was killed by SCP-173-J as soon as the disruptions in the electrical network started."
													Case "room2doors"
														DeathMSG = Chr(34)+"If I'm not mistaken, one of the main purposes of these rooms was to stop SCP-173-J from moving further in the event of a containment breach. "
														DeathMSG = DeathMSG + "So, who's brilliant idea was it to put A GODDAMN MAN-SIZED VENTILATION DUCT in there?"+Chr(34)
													Default 
														DeathMSG = "Subject D-9341. Cause of death: Fatal cervical fracture. Assumed to be attacked by SCP-173."
												End Select
			...
			Case NPCtypeIkobot ;------------------------------------------------------------------------------------------------------------------
				...
				n\ManipulateBone = False
				n\ManipulationType = 0
				n\NPCNameInSection = "Ikobot"
				
				Select n\State
				    Case 1 ; Follows the player (Fixed)					
						...
						If dist > 2.0 Or dist < 0.6 Then ; 1.0
							n\CurrSpeed = CurveValue(n\Speed*Sgn(dist-1.5)*0.75, n\CurrSpeed, 10.0)
						Else
							n\CurrSpeed = CurveValue(0, n\CurrSpeed, 10.0)
							n\State = 0
						...
						;	n\CurrSpeed = CurveValue(-n\Speed, n\CurrSpeed, 5.0)											
						;Else
						;	n\CurrSpeed = CurveValue(0, n\CurrSpeed, 30.0)
						;	If Int(AnimTime(n\obj))=143 Then
						;		n\CurrSpeed = 0
						;	ElseIf Int(AnimTime(n\obj))=130 Then
						;		n\CurrSpeed = 0
						;	EndIf
						;EndIf
						
						If n\CurrSpeed > 0 Then
							AnimateNPC(n, 121, 150, n\CurrSpeed*26)
						Else
							AnimateNPC(n, 150, 121, n\CurrSpeed*26)
						EndIf
...
Function UpdateMTFUnit(n.NPCs)
	...
	If n\BlinkTimer<=-5.0 Then 
		...
		If n\State = 2
			If OtherNPCSeesMeNPC(Curr173,n) Or OtherNPCSeesMeNPC(Curr173J,n)
			...
			Case 0 ;wandering around
                ...
                If n\PathTimer<=0.0 Then ;update path
					...
					Else ;i am the leader
						If Curr173\Idle<>2 Or Curr173J\Idle<>2
							For r = Each Rooms
						...
						Else
							Local tmp = False
							If EntityDistance(n\Collider,Curr173\Collider)>4.0 Or EntityDistance(n\Collider,Curr173J\Collider)>4.0
								If (Not EntityVisible(n\Collider,Curr173\Collider)) Or (Not EntityVisible(n\Collider,Curr173J\Collider))
									tmp = True
								EndIf
							EndIf
							...
							If (Not tmp)
								For r = Each Rooms
									If r\RoomTemplate\Name$ = "start"
										...
										If Curr173\Idle = 3 Or Curr173J\Idle = 3 And Distance(EntityX(pvt%),EntityZ(pvt%),EntityX(n\Collider),EntityZ(n\Collider)) > 4.0
...
Function Console_SpawnNPC(c_input$, c_state$ = "")
	...
	Select c_input$ 
		...
		Case "080", "scp080", "scp-080"
			n.NPCs = CreateNPC(NPCtype080, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "SCP-080 spawned."
		...
		Case "173j", "scp173j", "scp-173-J", "joke statue"
			n.NPCs = CreateNPC(NPCtype173J, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			Curr173J = n
			If (Curr173J\Idle = 3) Then Curr173J\Idle = False
			consoleMSG = "SCP-173-J spawned."

		Case "ikobot"
			n.NPCs = CreateNPC(NPCtypeIkobot, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Ikobot spawned."
...
Function NPCSpeedChange(n.NPCs)
	...
	Select n\NPCtype
		Case NPCtype173,NPCtypeOldMan,NPCtype096,NPCtype049,NPCtype939,NPCtypeMTF,NPCtype173J 
