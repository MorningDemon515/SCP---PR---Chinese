...
Function UpdateMainMenu()
		...
		For i% = 0 To 3
			...
			Else
				If hover And (Rand(20)=1) Then
					...
					Text (GraphicWidth/2)+Rand(-10,10)*MenuScale,y+Rand(-10,10)*MenuScale,txt,True,False
				EndIf
				Color 0,0,0
				Text (GraphicWidth/2)+3*MenuScale,y+3*MenuScale,txt,True,False
				If Not hover Then
					Color 255,255,255
				Else
					Color 100,100,150
				...
				Text (GraphicWidth/2),y,txt,True,False
			EndIf
	...
	Else
		...
		If (Not IS_3DMENU_ENABLED) Then		
			...
			back = DrawButton(x + width + 20 * MenuScale, y, 580 * MenuScale - width - 20 * MenuScale, height, "返回", False)
		Else If MainMenuTab<>8
			Color 0,0,0
			Text (GraphicWidth/2)+(420*MenuScale)+(3*MenuScale)-StringWidth("返回"),y+(3*MenuScale),"返回",False,False
			If MouseOn((GraphicWidth/2)+(420*MenuScale)-StringWidth("返回"), y, StringWidth("返回"), StringHeight("返回")) Then
				Color 100,100,150
			...
			Text (GraphicWidth/2)+(420*MenuScale)-StringWidth("返回"),y,"返回",False,False
		EndIf
		...
		Select MainMenuTab
			Case 1 ; New game
				...
				SetFont Font2
				If (Not IS_3DMENU_ENABLED) Then				
				    Text(x + width / 2, y + height / 2, "新的游戏", True, True)
				Else
					Text(GraphicWidth/2, y + height / 2, "新的游戏", True, True)
				...
				Else
					Text (x + 20 * MenuScale, y + 60 * MenuScale+3, "选择地图:")
					Color (255, 255, 255)
					...
					If DrawButton(x+370*MenuScale, y+55*MenuScale, 120*MenuScale, 30*MenuScale, "删除", False) Then
						SelectedMap=""
					EndIf
				EndIf	
				...
				If SelectedDifficulty\customizable Then
					...
					Text(x + 200 * MenuScale, y + 165 * MenuScale+3, "永久性死亡")
					...
					Select SelectedDifficulty\otherFactors
						Case EASY
							Text(x + 200 * MenuScale, y + 255 * MenuScale+3, "其他困难条件: 简单")
						Case NORMAL
							Text(x + 200 * MenuScale, y + 255 * MenuScale+3, "其他困难条件: 普通")
						Case HARD
							Text(x + 200 * MenuScale, y + 255 * MenuScale+3, "其他困难条件: 困难")
					End Select
				Else
			...
			Case 2 ;load game
				...
				SetFont Font2
				If (Not IS_3DMENU_ENABLED) Then				
				Text(x + width / 2, y + height / 2, "加载游戏", True, True)
				Else
					Text(GraphicWidth/2, y + height / 2, "加载游戏", True, True)
				EndIf				
				...
				If (Not IS_3DMENU_ENABLED) Then
					DrawFrame(x+50*MenuScale,y+510*MenuScale,width-100*MenuScale,55*MenuScale)
				    Text(x+(width/2.0),y+536*MenuScale,"页 "+Int(Max((CurrLoadGamePage+1),1))+"/"+Int(Max((Int(Ceil(Float(SaveGameAmount)/6.0))),1)),True,True)
				Else
				    Color(255, 255, 255)				
				    Text(GraphicWidth/2.0,y+536*MenuScale,"页 "+Int(Max((CurrLoadGamePage+1),1))+"/"+Int(Max((Int(Ceil(Float(SaveGameAmount)/6.0))),1)),True,True)									
				EndIf
				...
				If SaveGameAmount = 0 Then
					Text (x + 20 * MenuScale, y + 20 * MenuScale, "没有存档.")
				Else
					x = x + 20 * MenuScale
					y = y + 20 * MenuScale
					...
					If SaveMSG <> ""
						...
						RowText("是否确实要删除此存档?", x + 20 * MenuScale, y + 15 * MenuScale, 400 * MenuScale, 200 * MenuScale)
			...
			Case 3,5,6,7 ;options
				...
				SetFont Font2
				If (Not IS_3DMENU_ENABLED) Then				
				    Text(x + width / 2, y + height / 2, "设置", True, True)
				Else				
					Text(GraphicWidth/2, y + height / 2, "设置", True, True)
				EndIf				
				...
				Color 255,255,255
				If DrawButton(x+20*MenuScale,y+15*MenuScale,width/5,height/2, "图像", False) Then MainMenuTab = 3
				...
				If MainMenuTab = 3 ;Graphics
					...
					Color 255,255,255
					Text(x + 20 * MenuScale, y+5, "启用房间灯光:")
					...
					Color 255,255,255
					Text(x + 20 * MenuScale, y+3, "纹理LOD偏移:")
					...
					Color 255,255,255
					Text(x + 20 * MenuScale, y+5, "将纹理保存在VRAM:")
				...
				ElseIf MainMenuTab = 5 ;Audio
					...
					Color 255,255,255
					Text x + 20 * MenuScale, y+2, "声音自动播放:"
					...
					Color 255,255,255
					Text x + 20 * MenuScale, y+2, "启用用户跟踪:"
					...
					If EnableUserTracks
						y = y + 30 * MenuScale
						Color 255,255,255
						Text x + 20 * MenuScale, y+3, "用户跟踪模式:"
						UserTrackMode = DrawTick(x + 310 * MenuScale, y + MenuScale, UserTrackMode)
						If UserTrackMode
							Text x + 350 * MenuScale, y + 3 + MenuScale, "重复"
						Else
							Text x + 350 * MenuScale, y + 3 + MenuScale, "随机的"
						EndIf
						If MouseOn(x+310*MenuScale,y+MenuScale,20*MenuScale,20*MenuScale)
							DrawOptionsTooltip(tx,ty,tw,th,"usertrackmode")
						EndIf
						If DrawButton(x + 20 * MenuScale, y + 30 * MenuScale + 1, 190 * MenuScale, 25 * MenuScale, "扫描用户曲目",False)
						...
						EndIf
						If UserTrackCheck%>0
							Text x + 20 * MenuScale, y + 100 * MenuScale, "找到用户曲目 ("+UserTrackCheck2+"/"+UserTrackCheck+" successfully loaded)"
						EndIf
					Else
						UserTrackCheck%=0
					EndIf
				...
				ElseIf MainMenuTab = 7 ;Advanced
					...
					Color 255,255,255
					Text(x + 20 * MenuScale, y+3, "开启控制台并显示错误:")
					...
					Color 255,255,255
					Text(x + 20 * MenuScale, y+3, "成就弹出窗口:")
			...
			Case 4 ; load map
				...
				SetFont Font2
				If (Not IS_3DMENU_ENABLED) Then				
				    Text(x + width / 2, y + height / 2, "加载地图", True, True)
				Else
					Text(GraphicWidth/2, y + height / 2, "加载地图", True, True)
				EndIf
				...
				If (Not IS_3DMENU_ENABLED) Then
					DrawFrame(x+50*MenuScale,y+510*MenuScale,width-100*MenuScale,55*MenuScale)
				    Text(x+(width/2.0),y+536*MenuScale,"页 "+Int(Max((CurrLoadGamePage+1),1))+"/"+Int(Max((Int(Ceil(Float(SavedMapsAmount)/6.0))),1)),True,True)					
				Else
				    Color(255, 255, 255)				
				    Text(GraphicWidth/2.0,y+536*MenuScale,"页 "+Int(Max((CurrLoadGamePage+1),1))+"/"+Int(Max((Int(Ceil(Float(SavedMapsAmount)/6.0))),1)),True,True)										
				EndIf
				...
				If SavedMaps(0)="" Then 
					Text (x + 20 * MenuScale, y + 20 * MenuScale, "没有地图,使用地图制作器制作地图.")
				Else
					x = x + 20 * MenuScale
					y = y + 20 * MenuScale
					For i = (1+(6*CurrLoadGamePage)) To 6+(6*CurrLoadGamePage)
	...
	Text 20,GraphicHeight-30,"基于SCP:CB v1.3.11"	
...
Function FPSMainMenu(fpsamount%)
	...
	SetFont ConsoleFont	
	If ShowFPS Then Text 20, GraphicHeight-70, "FPS: " + fpsamount
...
Function UpdateLauncher()
	...
	LauncherIMG = LoadImage_Strict("ProjectR\GFX\menu\launcher.jpg")		
	...
	Repeat
		...
		Color 255, 255, 255
		Text(40 + 430 + 15, 262 - 55 + 35 - 8 + 2, "无边窗口",False,False)
		Text(40 + 430 + 15, 262 - 55 + 35 + 12 + 2, "窗口模式",False,False)
		...
		If (Not BorderlessWindowed)
			If Fullscreen
				Text(40+ 260 + 15, 262 - 55 + 140, "当前分辨率: "+(GfxModeWidths(SelectedGFXMode) + "x" + GfxModeHeights(SelectedGFXMode) + "," + (16+(16*(Not Bit16Mode)))))
			Else
				Text(40+ 260 + 15, 262 - 55 + 140, "当前分辨率: "+(GfxModeWidths(SelectedGFXMode) + "x" + GfxModeHeights(SelectedGFXMode) + ",32"))
			EndIf
		Else
			Text(40+ 260 + 15, 262 - 55 + 140, "当前分辨率: "+GfxModeWidths(SelectedGFXMode) + "x" + GfxModeHeights(SelectedGFXMode) + ",32")
			If GfxModeWidths(SelectedGFXMode)<G_viewport_width Then
				Text(40+ 260 + 65, 262 - 55 + 160, "(升级到")
...
Function DrawLoading(percent%, shortloading=False)
	...
	Repeat 
		...
		Color 0,0,0
		Text(GraphicWidth / 2 + 1, GraphicHeight / 2 - 100 + 1, "加载 - " + percent + " %", True, True)
		Color 255,255,255
		Text(GraphicWidth / 2, GraphicHeight / 2 - 100, "加载 - " + percent + " %", True, True)
...
Function DrawQuickLoading()
	...
	If QuickLoadPercent > -1
		...
		Color 255,255,255
		SetFont Font1
		Text GraphicWidth-100,GraphicHeight-90,"加载: "+QuickLoadPercent+"%",1
...
Function DrawOptionsTooltip(x%,y%,width%,height%,option$,value#=0,ingame%=False)
	...
	Select Lower(option$)
		;Graphic options
			;[Block]
		Case "bump"
			txt = Chr(34)+"凹凸贴图"+Chr(34)+" 用于通过扭曲光照贴图来模拟凹凸."
			txt2 = "此选项无法在游戏中更改."
			R = 255
		Case "vsync"
			txt = Chr(34)+"垂直同步"+Chr(34)+" 在计算下一帧之前，等待显示器完成当前刷新周期，以防止出现以下问题 "
			txt = txt + "屏幕撕裂。这会将游戏的帧速率与显示器的刷新率联系起来，并可能导致一些输入滞后."
		Case "antialias"
			txt = Chr(34)+"抗锯齿"+Chr(34)+" 用于在显示之前平滑渲染图像，以减少模型边缘的混叠."
			txt2 = "此选项仅在全屏模式下生效."
			R = 255
		Case "roomlights"
			txt = "切换在特定光源上生成的人造镜头光斑效果."
		Case "gamma"
			txt = Chr(34)+"亮度矫正"+Chr(34)+" 用于在游戏看起来太暗或太亮时实现良好的亮度因子，以平衡显示器的亮度. "
			txt = txt + "将其设置得过高或过低可能会导致图形看起来不那么详细."
			R = 255
			G = 255
			B = 255
			txt2 = "当前值: "+Int(value*100)+"% (default is 100%)"			
		Case "texquality"
			txt = Chr(34)+"纹理LOD偏移"+Chr(34)+" 影响纹理细节更改的距离以防止混叠。如果纹理闪烁或看起来过于模糊，请更改此选项."
		Case "particleamount"
			txt = "确定每个刻度可以渲染的粒子数量."
			Select value
				Case 0
