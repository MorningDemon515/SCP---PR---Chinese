			...
			ElseIf conlen > 0 Then
				...
				Repeat
					...
					If avail > DOWNLOAD_SIZE Then
						...
						Color 255,255,255
						Text 5,165,Str(Floor(((Float(pos)/1024.0)/1024.0)*100.0)/100.0)+"MB(共MB) "+Str(Floor(((Float(conlen)/1024.0)/1024.0)*100.0)/100.0)+"MB已下载"
...
Function CheckForUpdates%()
	...
	If latest<>VersionNumber And latest<>"" Then ;a new version is available!
		...
		UpdaterFont = LoadFont("GFX\font\cour\Morning Demon.ttf",16)
		...
		Repeat
			...
			If DownloadURL="" Then
				...
				Rect(480, 200, 155, 95)
				Color 0,0,0
				RowText2("但是,需要手动下载才能更新游戏.",482,210,150,90)
				
				SetFont Font1
				If DrawButton(LauncherWidth - 30 - 90, LauncherHeight - 50 - 110, 100, 30, "再试一次", False, False, False)
					Delete Each ChangeLogLines
					If UpdaterIMG <> 0 Then FreeImage UpdaterIMG
					CheckForUpdates()
					Return
				EndIf
				If DrawButton(LauncherWidth - 55 - 90, LauncherHeight - 50 - 55, 145, 30, "访问网站", False, False, False)
					ExecFile("http://scpcbgame.com")
					
					Delay 100
					End
				EndIf
				If DrawButton(LauncherWidth - 30 - 90, LauncherHeight - 50, 100, 30, "忽略", False, False, False)
					Delay 100
					Exit
				EndIf
			Else
				...
				SetFont Font1
				If DrawButton(LauncherWidth - 30 - 90, LauncherHeight - 50 - 55, 100, 30, "更新", False, False, False)
					ConfirmDownload=True
					Cls
					Color 255,255,255
					Text 320,240,"开始下载...",True,True
					Flip
					Delay 100
					Exit
				EndIf
