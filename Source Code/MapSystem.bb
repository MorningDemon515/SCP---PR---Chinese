...
LoadRoomTemplates("ProjectR\Data\rooms.ini")
...
;			EntityParent dp\cam,fr\Forest_Pivot
		...
		Case "room079"
			...
			;Bloody Note
			it = CreateItem("Bloody Note", "paper", r\x + 1182.0 * RoomScale, r\y - 445.0 * RoomScale, r\z + 1792.0 * RoomScale)
			EntityParent(it\collider, r\obj)			
			;[End Block]
		Case "checkpoint1"
			;[Block]
		...
		Case "room2poffices"
			...
			it = CreateItem("Note To Doctor Harp", "paper", r\x + 706.0 * RoomScale, r\y + 183.0 * RoomScale, r\z - 576.0 * RoomScale)
			EntityParent(it\collider, r\obj)
...
;			PositionEntity(r\Objects[6], r\x - 848*RoomScale, 0.5, r\z-576*RoomScale, True)
		...
		Case "room2test1074"
			...
			r\Textures[0] = LoadTexture("ProjectR\GFX\map\1074tex0.jpg") ;blank texture (ripped from official article), seen when you put on 714
			r\Textures[1] = LoadTexture("ProjectR\GFX\map\1074tex1.jpg") ;texture depicting subject D-9341 (the player)
			r\Textures[2] = LoadTexture("ProjectR\GFX\map\1074tex2.jpg") ;test texture for 113 condition
						
			TextureBlend r\Textures[0], 5 ;texture
			TextureBlend r\Textures[1], 5 ;blends
			TextureBlend r\Textures[2], 5 ;effect?
		...
		Case "room2sl"
			...
			r\Textures[2] = LoadTexture("ProjectR\GFX\SL_409.jpg") ;409 BoH
				...
				If i <> 7 Then
					...
					If i <> 4 And i <> 13 Then
						...
						Select i
							...
							Case 1
								EntityTexture screen,r\Textures[2] ;409								
							Case 2
...
;			RotateEntity r\Objects[21],8.6,-150.0,-5.0
		...
		Case "newroom005"
			...
			r\Objects[0]= CreatePivot(r\obj)
			PositionEntity(r\Objects[0], r\x + 0.0 * RoomScale, r\y + 76.0 * RoomScale, r\z + 768.0 * RoomScale, True)
			EntityParent r\Objects[0],r\obj

			r\Objects[1]= CreatePivot(r\obj)
			;PositionEntity(r\Objects[1], r\x + 0.0 * RoomScale, r\y + 172.0 * RoomScale, r\z + 1008.0 * RoomScale, True)
			PositionEntity(r\Objects[1], r\x + 0.0 * RoomScale, r\y + 188.0 * RoomScale, r\z + 1008.0 * RoomScale, True)			
			EntityParent r\Objects[1],r\obj

			r\Objects[2]= CreatePivot(r\obj)
			;PositionEntity(r\Objects[2], r\x + 0.0 * RoomScale, r\y + 172.0 * RoomScale, r\z + 1064.0 * RoomScale, True)
			PositionEntity(r\Objects[2], r\x + 0.0 * RoomScale, r\y + 12.0 * RoomScale, r\z + 1120.0 * RoomScale, True)			
			EntityParent r\Objects[2],r\obj

			r\Objects[3]= CreatePivot(r\obj)
			;PositionEntity(r\Objects[3], r\x + 0.0 * RoomScale, r\y - 20.0 * RoomScale, r\z + 928.0 * RoomScale, True)
			PositionEntity(r\Objects[3], r\x + 0.0 * RoomScale, r\y + 12.0 * RoomScale, r\z + 928.0 * RoomScale, True)			
			EntityParent r\Objects[3],r\obj
		...
		Case "room009"
			...
			r\Textures[0]=LoadTexture("ProjectR\GFX\map\room009_lm1b.png",1)
			TextureBlend r\Textures[0],3
			TextureCoords r\Textures[0],1
			r\Textures[1]=LoadTexture("ProjectR\GFX\map\room009_lm1.png",1)
		...
		Case "room409"
			...
			r\Objects[1] = LoadAnimMesh_Strict("ProjectR\GFX\npcs\zombie2.b3d")
			...
			Local CrystalZombie = LoadTexture_Strict("ProjectR\GFX\npcs\zombie2.jpg")
			...
			r\RoomDoors[1] = CreateDoor(r\zone, r\x - 2328.0 * RoomScale, -4520.0 * RoomScale, r\z - 656.0 * RoomScale, 90, r, False, 3)
			r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = False	
            ...
            sc.SecurityCams = CreateSecurityCam(r\x - 3624.0 * RoomScale, r\y  - 4100.0 * RoomScale, r\z + 2248.0 * RoomScale, r)
			sc\angle = 100
			sc\turn = 45
			;CameraRange sc\Cam, 20.05, 30.0
			TurnEntity(sc\CameraObj, 20, 0, 0)
			;sc\ID = 11 And 12
		...
		Case "room2testmisc"
			...
			temp% = Rand(1,5)
			r\RoomDoors[0] = CreateDoor(r\zone,r\x,r\y,r\z,0,r,False,False,False,"")
			;r\RoomDoors[0]\Locked = 2
			r\RoomDoors[1] = CreateDoor(r\zone,r\x + 336.0 * RoomScale,r\y,r\z + 671.0 * RoomScale,90,r,False,False,temp)
			;r\RoomDoors[1]\AutoClose = False
			r\RoomDoors[2] = CreateDoor(r\zone,r\x + 336.0 * RoomScale,r\y,r\z - 800.0 * RoomScale,90,r,False,False,temp)
			;r\RoomDoors[2]\AutoClose = False
			r\RoomDoors[3] = CreateDoor(r\zone,r\x + 672.0 * RoomScale,r\y,r\z,0,r,False,False)
			;[End Block]
		Case "oldmedibay"
			...
			r\Objects[3]=-1
			;[End Block]
		Case "room001" ;001's room	
			;[Block]

            ;Gates
			r\RoomDoors[0] = CreateDoor(r\zone,r\x,0,r\z + 352.0*RoomScale,0,r,False,True,5)
			r\RoomDoors[0]\dir=1 : r\RoomDoors[0]\AutoClose=False : r\RoomDoors[0]\open=False
			PositionEntity (r\RoomDoors[0]\buttons[0], r\x + 235.0 * RoomScale, 0.7, r\z + 268.0 * RoomScale, True)
			TurnEntity(r\RoomDoors[0]\buttons[0], 0, 180, 0)
			PositionEntity (r\RoomDoors[0]\buttons[1], r\x + 235.0 * RoomScale, 0.7, r\z + 456.0 * RoomScale, True)
			TurnEntity(r\RoomDoors[0]\buttons[1], 0, 360, 0)
						
            ;Doors
			r\RoomDoors[1] = CreateDoor(r\zone, r\x + 794.0 * RoomScale, r\y - 224.0 * RoomScale, r\z + 770.0 * RoomScale, 90, r, False, False)
			PositionEntity(r\RoomDoors[1]\buttons[0], r\x + 832.0 * RoomScale, r\y - 45 * RoomScale, r\z + 930.0 * RoomScale, True)	
			RotateEntity(r\RoomDoors[1]\buttons[0],0,360,0)	;Clear					
			PositionEntity(r\RoomDoors[1]\buttons[1], r\x + 730.0 * RoomScale, r\y - 45 * RoomScale, r\z + 674.0 * RoomScale, True)
			RotateEntity(r\RoomDoors[1]\buttons[1],0,90,0) ;Clear			

            ;Control Room
			r\RoomDoors[2] = CreateDoor(r\zone, r\x + 1122.0 * RoomScale, r\y - 224.0 * RoomScale, r\z + 1103.0 * RoomScale, 180, r, False, False)
			PositionEntity(r\RoomDoors[2]\buttons[0], r\x + 1260.0 * RoomScale, r\y - 45 * RoomScale, r\z + 1065.0 * RoomScale, True)
			RotateEntity(r\RoomDoors[2]\buttons[0],0,180,0) ;Clear		
			PositionEntity(r\RoomDoors[2]\buttons[1], r\x + 976.0 * RoomScale, r\y - 45 * RoomScale, r\z + 1142.0 * RoomScale, True)
			RotateEntity(r\RoomDoors[2]\buttons[1],0,0,0) ;Clear			
			
			;Chamber
			r\RoomDoors[3] = CreateDoor(r\zone, r\x + 1450.0 * RoomScale, r\y - 224.0 * RoomScale, r\z + 870.0 * RoomScale, 90, r, False, False)
			PositionEntity(r\RoomDoors[3]\buttons[0], r\x + 1410.0 * RoomScale, r\y - 45 * RoomScale, r\z + 726.0 * RoomScale, True) ;Need fix
			RotateEntity(r\RoomDoors[3]\buttons[0],0,180,0) ;Clear			
			PositionEntity(r\RoomDoors[3]\buttons[1], r\x + 1490.0 * RoomScale, r\y - 45 * RoomScale, r\z + 1016.0 * RoomScale, True) ;Need fix
			RotateEntity(r\RoomDoors[3]\buttons[1],0,360,0) ;Clear			
									
			;Briefcase
			r\Objects[0] = LoadAnimMesh_Strict("ProjectR\GFX\map\Case.b3d")
			ScaleEntity(r\Objects[0], 0.0024, 0.0024, 0.0024, True)
			PositionEntity (r\Objects[0], r\x + 2406.0*RoomScale,r\y-64.0*RoomScale, r\z+1262.0*RoomScale, True)			
			EntityParent(r\Objects[0], r\obj)
			TurnEntity r\Objects[0],0,90,0,True
						
			;PositionEntity (r\Objects[1], r\x + 2403.0, r\y - 26.0 * RoomScale, r\z + 1262.0 * RoomScale, True)						
            ;SetAnimTime(r\Objects[0], 3.0)
									
			;ScaleEntity(r\Objects[0], 5 * RoomScale, 5 * RoomScale, 5 * RoomScale)
			;EntityPickMode(r\Objects[0], 2)
			
			;Items
			it = CreateItem("SCP-001","scp001",r\x + 2408.0 * RoomScale,r\y - 46.0*RoomScale,r\z + 1264.0*RoomScale)
			RotateEntity it\collider,0,0,0			
			EntityParent(it\collider, r\obj)
			
			sc.SecurityCams = CreateSecurityCam(r\x + 1519.0 * RoomScale, r\y + 416 * RoomScale, r\z + 1808.0 * RoomScale, r, True)
			sc\angle = -145
			;sc\turn = 45						
			TurnEntity(sc\CameraObj, 20, 0, 0)
			EntityParent(sc\obj, r\obj)
			...
			r\Objects[1] = sc\CameraObj
			r\Objects[2] = sc\obj
			
			PositionEntity(sc\ScrObj, r\x + 856.0 * RoomScale, 38.0 * RoomScale, r\z + 1322.0 * RoomScale)
			TurnEntity(sc\ScrObj, 0, 90, 0)
			EntityParent sc\ScrObj, r\obj
			sc\CoffinEffect=0
						
			;Document SCP-001 Report
			;it = CreateItem("Document SCP-650","paper",r\x - 502.0 * RoomScale,r\y + 122.0 * RoomScale,r\z - 64.0 * RoomScale)
			;RotateEntity it\collider,0,-145,0				
			;EntityParent(it\collider, r\obj)																																							
			;[End Block]
		Case "room006"		
			;[Block]
			;d = CreateDoor(r\zone, r\x - 208.0 * RoomScale, r\y, r\z - 160.0 * RoomScale, 90, r,False, False, 5)			
			;d = CreateDoor(r\zone, r\x - 224.0 * RoomScale, r\y, r\z - 192.0 * RoomScale, 90, r,False, False, 5)
			
			d = CreateDoor(r\zone, r\x - 224.0 * RoomScale, r\y - 3935.0 * RoomScale, r\z - 192.0 * RoomScale, 90, r,False, False, 5)			
			d\open = False : d\locked = False : d\AutoClose = False

			r\RoomDoors[0] = CreateDoor(r\zone,r\x - 640.0 * RoomScale,r\y - 3935.0 * RoomScale,r\z + 32.0 * RoomScale,0,r,False,False)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = False
			
			;r\RoomDoors[1] = CreateDoor(r\zone, r\x, r\y, r\z + 736.0 * RoomScale, 0, r, False, False)
			r\RoomDoors[1] = CreateDoor(r\zone, r\x, r\y - 3935.0 * RoomScale, r\z + 736.0 * RoomScale, 0, r, False, False)			
			PositionEntity(r\RoomDoors[1]\buttons[0], r\x + 580.822 * RoomScale, EntityY(r\RoomDoors[1]\buttons[0],True), r\z - 606.679 * RoomScale, True)	
            PositionEntity(r\RoomDoors[1]\buttons[1], r\x + 580.822 * RoomScale, EntityY(r\RoomDoors[1]\buttons[1],True), r\z - 606.679 * RoomScale, True)
			r\RoomDoors[1]\dir = 0 : r\RoomDoors[1]\AutoClose = False	: r\RoomDoors[1]\open = True  : r\RoomDoors[1]\locked = True	
			r\RoomDoors[1]\MTFClose = False
			
			;r\RoomDoors[2] = CreateDoor(r\zone, r\x, r\y, r\z + 1376.0 * RoomScale, 180, r, False, False)
			r\RoomDoors[2] = CreateDoor(r\zone, r\x, r\y - 3935.0 * RoomScale, r\z + 1376.0 * RoomScale, 180, r, False, False)			
			PositionEntity(r\RoomDoors[2]\buttons[0], r\x + 580.822 * RoomScale, EntityY(r\RoomDoors[2]\buttons[0],True), r\z - 606.679 * RoomScale, True)	
            PositionEntity(r\RoomDoors[2]\buttons[1], r\x + 580.822 * RoomScale, EntityY(r\RoomDoors[2]\buttons[1],True), r\z - 606.679 * RoomScale, True)
			r\RoomDoors[2]\dir = 0 : r\RoomDoors[2]\AutoClose = False	: r\RoomDoors[2]\open = True  : r\RoomDoors[2]\locked = True
			r\RoomDoors[2]\MTFClose = False
						
			r\Objects[0] = CreatePivot()
			PositionEntity r\Objects[0],r\x+304.0*RoomScale,-3808.0*RoomScale,r\z+978.0*RoomScale ;968
			EntityParent r\Objects[0],r\obj	

			r\RoomDoors[3] = CreateDoor(r\zone, r\x - 320.0 * RoomScale, 0.0, r\z + 0.0 * RoomScale, 90, r, True, 3)
			r\RoomDoors[3]\AutoClose = False : r\RoomDoors[3]\open = True
			PositionEntity(r\RoomDoors[3]\buttons[0], r\x - 284.0 * RoomScale, 0.7, r\z + 140.0 * RoomScale, True)
			PositionEntity(r\RoomDoors[3]\buttons[1], r\x - 356.0 * RoomScale, 0.7, r\z - 180.0 * RoomScale, True)			
			
			r\RoomDoors[4] = CreateDoor(r\zone, r\x - 264.0 * RoomScale, r\y - 3935.0 * RoomScale, r\z - 784.0 * RoomScale, 90, r, False, 3)
			r\RoomDoors[4]\AutoClose = False : r\RoomDoors[4]\open = False	
			PositionEntity(r\RoomDoors[4]\buttons[0], r\x - 228.0 * RoomScale, EntityY(r\RoomDoors[4]\buttons[0],True), r\z - 608.0 * RoomScale, True)
			PositionEntity(r\RoomDoors[4]\buttons[1], r\x - 300.0 * RoomScale, EntityY(r\RoomDoors[4]\buttons[1],True), r\z - 964.0 * RoomScale, True)

			r\Objects[5] = CreatePivot()
			PositionEntity(r\Objects[5], r\x - 632.0 * RoomScale, 240.0 * RoomScale, r\z + 0.0 * RoomScale)
			EntityParent(r\Objects[5], r\obj)
			
			r\Objects[6] = CreatePivot()
			PositionEntity(r\Objects[6], r\x - 576.0 * RoomScale, -3806.0 * RoomScale, r\z - 784.0 * RoomScale)
			EntityParent(r\Objects[6], r\obj)								

		    r\Objects[3] = CopyEntity(LeverBaseOBJ)
			r\Objects[4] = CopyEntity(LeverOBJ)
			
			r\Levers[0] = r\Objects[4]
			
			For  i% = 0 To 1
				ScaleEntity(r\Objects[3 + i], 0.04, 0.04, 0.04)
				PositionEntity r\Objects[3 + i],r\x-568*RoomScale,r\y-3712.0*RoomScale,r\z+970*RoomScale,True
				
				EntityParent(r\Objects[3 + i], r\obj)
			Next
			RotateEntity(r\Objects[3], 0, 0, 0)
			RotateEntity(r\Objects[4], 10, 0 - 180, 0)
				
			EntityPickMode r\Objects[4], 1, False
			EntityRadius r\Objects[4], 0.1

			r\Objects[1] = CopyEntity(LeverBaseOBJ)
			r\Objects[2] = CopyEntity(LeverOBJ)
			
			For  i% = 0 To 1
				ScaleEntity(r\Objects[1 + i], 0.04, 0.04, 0.04)
				PositionEntity r\Objects[1],r\x-644.0*RoomScale, r\y-3712.0*RoomScale, r\z+970.0*RoomScale,True ; Z - 975.0
				PositionEntity r\Objects[2],r\x-644.0*RoomScale, r\y-3712.0*RoomScale, r\z+970.0*RoomScale,True
				
				EntityParent(r\Objects[1 + i], r\obj)
			Next
			
			RotateEntity r\Objects[2],106.0,0.0,180.0

			it = CreateItem("Document SCP-006", "paper", r\x - 1006.0 * RoomScale, r\y - 3809.0 * RoomScale, r\z + 832.0 * RoomScale)		
			EntityParent(it\collider, r\obj)
			...
			;[End Block]
		Case "room1warehouse" ;Standard Storage with Mini warehouse	
			;[Block]
			;Doors/Gates
			d = CreateDoor(r\zone,r\x - 496.0 * RoomScale,r\y + 0.0 * RoomScale,r\z - 416.0 * RoomScale,0,r,False,False, 2)
			d = CreateDoor(r\zone,r\x + 496.0 * RoomScale,r\y + 0.0 * RoomScale,r\z - 416.0 * RoomScale,0,r,False,False, 3)

			r\RoomDoors[0] = CreateDoor(r\zone, r\x+992.0 * RoomScale,0,r\z+2128.0 * RoomScale, 0, r, False, True, 6)
			r\RoomDoors[0]\AutoClose=False : r\RoomDoors[0]\open=False
			FreeEntity (r\RoomDoors[0]\buttons[0]) : r\RoomDoors[0]\buttons[0] = 0
			FreeEntity (r\RoomDoors[0]\buttons[1]) : r\RoomDoors[0]\buttons[1] = 0
						
			;Items
			it = CreateItem("Document SCP-113","paper",r\x - 864.0 * RoomScale,r\y+8.0*RoomScale,r\z - 224.0*RoomScale)
			RotateEntity it\collider,0,70,0				
			EntityParent(it\collider, r\obj)
			
			;SCP-113
			r\Objects[0] = CreatePivot()
			PositionEntity r\Objects[0],r\x-865.0*RoomScale,r\y+126.0*RoomScale,r\z+640.0*RoomScale
			EntityParent r\Objects[0],r\obj
			EntityPickMode r\Objects[0],1

            ;SCP-1079's Packet
			r\Objects[1] = LoadMesh_Strict("ProjectR\GFX\items\scp1079_packet.b3d")
			PositionEntity(r\Objects[1], r\x + 651.0 * RoomScale, r\y + 7.0 * RoomScale, r\z + 1525.0 * RoomScale, 0)
			EntityParent(r\Objects[1], r\obj)
			ScaleEntity r\Objects[1], 45.0, 45.0, 45.0						
			RotateEntity r\Objects[1], 0, -20, 90
						
			r\Objects[2] = CopyEntity(r\Objects[1])			
			PositionEntity(r\Objects[2], r\x + 633.0 * RoomScale, r\y + 7.0 * RoomScale, r\z + 1512.0 * RoomScale, 0)
			EntityParent(r\Objects[2], r\obj)
			ScaleEntity r\Objects[2], 45.0, 45.0, 45.0
			RotateEntity r\Objects[2], 0, 0, 90						
			;TurnEntity r\Objects[1], 0, 180, 0

			r\Objects[3] = CopyEntity(r\Objects[1])			
			PositionEntity(r\Objects[3], r\x + 637.0 * RoomScale, r\y + 10.5 * RoomScale, r\z + 1526.0 * RoomScale, 0)
			EntityParent(r\Objects[3], r\obj)
			ScaleEntity r\Objects[3], 45.0, 45.0, 45.0
			RotateEntity r\Objects[3], 0, 25, 90						
			;TurnEntity r\Objects[1], 0, 180, 0

			r\Objects[4] = CopyEntity(r\Objects[1])			
			PositionEntity(r\Objects[4], r\x + 622.0 * RoomScale, r\y + 7.0 * RoomScale, r\z + 1546.0 * RoomScale, 0)
			EntityParent(r\Objects[4], r\obj)
			ScaleEntity r\Objects[4], 45.0, 45.0, 45.0
			RotateEntity r\Objects[4], 0, 10, 90						
			;TurnEntity r\Objects[1], 0, 180, 0

			r\Objects[5] = CopyEntity(r\Objects[1])			
			PositionEntity(r\Objects[5], r\x + 588.0 * RoomScale, r\y + 7.0 * RoomScale, r\z + 1551.0 * RoomScale, 0)
			EntityParent(r\Objects[5], r\obj)
			ScaleEntity r\Objects[5], 45.0, 45.0, 45.0
			RotateEntity r\Objects[5], 0, 35, 90						
			;TurnEntity r\Objects[1], 0, 180, 0

			r\Objects[6] = CopyEntity(r\Objects[1])			
			PositionEntity(r\Objects[6], r\x + 602.0 * RoomScale, r\y + 7.0 * RoomScale, r\z + 1570.0 * RoomScale, 0)
			EntityParent(r\Objects[6], r\obj)
			ScaleEntity r\Objects[6], 45.0, 45.0, 45.0
			RotateEntity r\Objects[6], 0, 10, 90						
			;TurnEntity r\Objects[1], 0, 180, 0

			r\Objects[7] = CopyEntity(r\Objects[1])			
			PositionEntity(r\Objects[7], r\x + 635.0 * RoomScale, r\y + 7.0 * RoomScale, r\z + 1561.0 * RoomScale, 0)
			EntityParent(r\Objects[7], r\obj)
			ScaleEntity r\Objects[7], 45.0, 45.0, 45.0
			RotateEntity r\Objects[7], 0, -10, 90						
			;TurnEntity r\Objects[1], 0, 180, 0

			r\Objects[8] = CopyEntity(r\Objects[1])			
			PositionEntity(r\Objects[8], r\x + 612.0 * RoomScale, r\y + 12.0 * RoomScale, r\z + 1570.0 * RoomScale, 0)
			EntityParent(r\Objects[8], r\obj)
			ScaleEntity r\Objects[8], 45.0, 45.0, 45.0
			RotateEntity r\Objects[8], 6.0, -60.5, 98						
			;TurnEntity r\Objects[1], 0, 180, 0

			r\Objects[9] = CopyEntity(r\Objects[1])			
			PositionEntity(r\Objects[9], r\x + 663.0 * RoomScale, r\y + 6.5 * RoomScale, r\z + 1548.0 * RoomScale, 0)
			EntityParent(r\Objects[9], r\obj)
			ScaleEntity r\Objects[9], 45.0, 45.0, 45.0
			RotateEntity r\Objects[9], -1.8, 9.8, 80						
			;TurnEntity r\Objects[1], 0, 180, 0

			r\Objects[10] = LoadMesh_Strict("ProjectR\GFX\map\079.b3d")			
			PositionEntity(r\Objects[10], r\x + 1928.0 * RoomScale, r\y + 6.0 * RoomScale, r\z - 272.0 * RoomScale, 0)
			EntityParent(r\Objects[10], r\obj)
			ScaleEntity r\Objects[10], 0.6, 0.6, 0.6
			RotateEntity r\Objects[10], 0, 180, 0						
			;TurnEntity r\Objects[1], 0, 180, 0
																														
			;SCP-1079's are colliding each other lol
			;it = CreateItem("SCP-1079","scp1079",r\x + 642.0 * RoomScale,r\y+5.0*RoomScale,r\z + 1507.0*RoomScale)
			;RotateEntity it\collider,0,-45,80				
			;EntityParent(it\collider, r\obj)			
			;it = CreateItem("SCP-1079","scp1079",r\x + 650.0 * RoomScale,r\y+7.0*RoomScale,r\z + 1547.0*RoomScale)
			;RotateEntity it\collider,0,-15,90				
			;EntityParent(it\collider, r\obj)						
			;it = CreateItem("SCP-1079","scp1079",r\x + 681.0 * RoomScale,r\y+7.0*RoomScale,r\z + 1511.0*RoomScale)
			;RotateEntity it\collider,0,-45,90				
			;EntityParent(it\collider, r\obj)			
			;it = CreateItem("SCP-1079","scp1079",r\x + 618.0 * RoomScale,r\y+7.0*RoomScale,r\z + 1567.0*RoomScale)
			;RotateEntity it\collider,0,-165,90				
			;EntityParent(it\collider, r\obj)
			
			;it = CreateItem("SCP-1079","scp1079",r\x + 657.0 * RoomScale,r\y+7.0*RoomScale,r\z + 1552.0*RoomScale)
			;RotateEntity it\collider,0,-20,90				
			;EntityParent(it\collider, r\obj)
			
			it = CreateItem("SCP-1079","scp1079",r\x + 658.0 * RoomScale,r\y+3.0*RoomScale,r\z + 1487.0*RoomScale)
			RotateEntity it\collider,0,-95,90 ;100				
		    EntityParent(it\collider, r\obj)																						
			;[End Block]
		Case "room650"			
		    ;[Block]
			...
			;Doors		
			r\RoomDoors[0] = CreateDoor(r\zone, r\x - 416.0 * RoomScale, r\y + 0.0 * RoomScale, r\z - 704.0 * RoomScale, 90, r, False, False)	
			r\RoomDoors[1] = CreateDoor(r\zone, r\x - 736.0 * RoomScale, r\y + 0.0 * RoomScale, r\z - 480.0 * RoomScale, 0, r, False, False, 4)
			r\RoomDoors[2] = CreateDoor(r\zone, r\x - 0.0 * RoomScale, r\y + 0.0 * RoomScale, r\z - 480.0 * RoomScale, 0, r, False, False, 2)

			;sc.SecurityCams = CreateSecurityCam(r\x - 353.0 * RoomScale, r\y + 580.0 * RoomScale, r\z - 393.0 * RoomScale, r, True)
			sc.SecurityCams = CreateSecurityCam(r\x - 337.0 * RoomScale, r\y + 580.0 * RoomScale, r\z - 368.0 * RoomScale, r, True)
			sc\angle = -45
			;sc\turn = 45						
			TurnEntity(sc\CameraObj, 20, 0, 0)
			EntityParent(sc\obj, r\obj)
			...
			r\Objects[1] = CopyEntity(Monitor)
			ScaleEntity r\Objects[1],scale#,scale#,scale#
			PositionEntity r\Objects[1],r\x - 1001 * RoomScale, r\y + 231.0 * RoomScale, r\z- 161.0 * RoomScale,True
			RotateEntity r\Objects[1],0,90,0
			EntityParent r\Objects[1],r\obj
			
            ;Pivots
			r\Objects[0] = CreatePivot(r\obj)
			PositionEntity(r\Objects[0], r\x + 320.0 * RoomScale, r\y + 0.0 * RoomScale, r\z + 72.0 * RoomScale, True)
			;PositionEntity(r\Objects[0], r\x + 320.0 * RoomScale, r\y + 16.0 * RoomScale, r\z + 72.0 * RoomScale, True)

			r\Objects[2] = CreatePivot(r\obj)
			PositionEntity(r\Objects[2], r\x + 399.0 * RoomScale, r\y + 0.0 * RoomScale, r\z + 143.0 * RoomScale, True)
			RotateEntity(r\Objects[2], 0, 45, 0)
									
			;Items
			it = CreateItem("Document SCP-650","paper",r\x - 496.0 * RoomScale,r\y + 126.0*RoomScale,r\z - 64.0*RoomScale)
			RotateEntity it\collider,0,-130,0			
			EntityParent(it\collider, r\obj)
		...
		If s\room = PlayerRoom Then
			If EntityDistance(Collider,s\obj)<1.2 Then
				...
				If PickedEntity()=s\obj And s\imgpath<>"" Then
					...
					If MouseUp1 Then 
						SelectedScreen=s
						If PlayerRoom\RoomTemplate\Name = "room2test1074" Then ;Any best alternative solutions?
						s\img = LoadImage_Strict("ProjectR\GFX\screens\"+s\imgpath)
						Else
		...
		Else
			...
			If close = True Then
				If sc\Screen Then
					...
					If Sanity < (-1000) And SCP173Mode = False Then 
...
Function CreateMap()
	...
	SetRoom("room1warehouse", ROOM1, Floor(0.7*Float(Room1Amount[0])),min_pos,max_pos)
    ...
    ;PR/BOH/FB
	SetRoom("room650", ROOM1, Room1Amount[0]+Floor(0.5*Float(Room1Amount[1])),min_pos,max_pos)
	...
	SetRoom("room006", ROOM2, Room2Amount[0]+Floor(0.8*Float(Room2Amount[1])),min_pos,max_pos)
	...
	;PR/BOH/FB
	SetRoom("room001", ROOM1, min_pos+Floor(0.1*Room1Amount[2]),min_pos,max_pos)
